<?php
	...
	if(! $conn){
		...
		$queryPersona .= "plw.LATITUDE, plw.LONGITUDE, p.info as generales,
				...
				pat.name as pacientesxsemana, 
				fee.NAME as honorarios, 
				...
				left outer join CODELIST fee on fee.CLIST_SNR = p.FEE_TYPE_SNR
...
//echo "<script>";
		echo "<script>
				...
				$('#lblEspecialidadAudiencia').text('".utf8_encode($reg['subespecialidad'])."');
				$('#lblPacientesDia').text('".utf8_encode($reg['pacientesxsemana'])."');
				$('#lblHonorarios').text('".utf8_encode($reg['honorarios'])."');
