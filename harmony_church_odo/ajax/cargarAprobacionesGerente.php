<?php
	...
	if(! $conn){
		...
		$query = "select top 1000 p.PERS_APPROVAL_SNR, p.P_MOVEMENT_TYPE, p.P_LNAME,p.P_MOTHERS_LNAME, p.P_FNAME, c.name as esp, 
		...
		while($aproba = sqlsrv_fetch_array($rs)){
			...
			$paterno = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['P_LNAME'])));
			$materno = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['P_MOTHERS_LNAME'])));
			$nombre = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['P_FNAME'])));
			$especialidad = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['esp'])));
			$institucion = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['institucion'])));
			$direccion = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['direccion'])));
			$colonia = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['colonia'])));
			$rutaU = str_replace("'", "\'", str_ireplace($buscar,$reemplazar,utf8_encode($aproba['ruta'])));
			
			
			if($estatus == 1){
				echo "$('#tblAprobacionesPersGerente tbody').append('<tr class=\"pointer\" onClick=\"muestraAprobacion(\'".$aproba['PERS_APPROVAL_SNR']."\',\'p\');\"><td style=\"width:5%;\">".$tipo."</td><td style=\"width:10%;\">".$paterno."</td><td style=\"width:10%;\">".$materno."</td><td style=\"width:11%;\">".$nombre."</td><td style=\"width:11%;\">".$especialidad."</td><td style=\"width:13%;\">".$institucion."</td><td style=\"width:15%;\">".$direccion."</td><td style=\"width:12%;\">".$colonia."</td><td style=\"width:8%;\" >".$fecha."</td><td style=\"width:5%;\">".$rutaU."</td></tr>');";
			}else{
				echo "$('#tblAprobacionesPersGerente tbody').append('<tr><td style=\"width:5%;\">".$tipo."</td><td style=\"width:10%;\">".$paterno."</td><td style=\"width:10%;\">".$materno."</td><td style=\"width:11%;\">".$nombre."</td><td style=\"width:11%;\">".$especialidad."</td><td style=\"width:13%;\">".$institucion."</td><td style=\"width:15%;\">".$direccion."</td><td style=\"width:12%;\">".$colonia."</td><td style=\"width:8%;\" >".$fecha."</td><td style=\"width:5%;\">".$rutaU."</td></tr>');";
			}
		}
		
		$query = "select top 1000 i.inst_approval_snr, i.i_movement_type, city.name as colonia, i.i_name, 
