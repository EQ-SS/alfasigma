...
if(! $conn){
	...
	if($idPersona != ''){
		$queryPersona = "select p.FNAME, 
			...
			pud.field_04_snr,
			pud.field_05_snr,
			case isnull(year(p.birthdate), 0) when 0 then 0 else year(getdate()) - year(p.birthdate) end as edad,
			right('00' + cast(day(p.creation_timestamp) as varchar),2) + '/' + right('00' + cast(month(p.creation_timestamp) as varchar), 2) + '/' + cast(year(p.creation_timestamp) as varchar) as fechaAlta
			from person p
		...
		$cedula = '';
		$frecuencia = '9F1F6A50-8EDB-42E2-98E6-F957CF8F23C6';
	...
	echo "<script>
		...
		$('#sltField01').val('".$field_01_snr."');
		$('#sltField02').val('".$field_02_snr."');
		$('#sltField03').val('".$field_03_snr."');
		$('#sltField04').val('".$field_04_snr."');
		$('#sltField05').val('".$field_05_snr."');
		$('#sltEstatusPersonaNuevo').val('".$estatus."');
		$('#sltDificultadVisita').val('".$dificultadVisita."');
		$('#txtEdad').val('".$edad."');
		$('#txtFechaAltaPersonaNuevo').val('".$fechaAlta."');
		";
