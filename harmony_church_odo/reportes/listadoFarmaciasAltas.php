<?php
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoFarmaciasAltas".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado Altas de Farmacias");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoFarmaciasAltas.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO ALTAS DE FARMACIAS'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO ALTAS DE FARMACIAS</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO ALTAS DE FARMACIAS')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoFarmaciasAltas');
