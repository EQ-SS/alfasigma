<?php
	...
	$qMedicos = "DECLARE @DIAS_IN AS DATE
		...
		SET @DIAS_FIN='".$fechaF."'
		/*
		SET @DIAS_IN='2019-05-06'
		SET @DIAS_FIN='2019-05-31'
		*/
		...
		And VP.visit_date Between @DIAS_IN And @DIAS_FIN
		and P.status_snr ='B426FB78-8498-4185-882D-E0DC381460E8'
		And VP.escort_snr in ('4AFE5D70-7759-47B8-9333-E9CF289D4582')
		) AS Vis_GteDto,
		...
		and VP.visit_date Between @DIAS_IN And @DIAS_FIN 
		and P.status_snr ='B426FB78-8498-4185-882D-E0DC381460E8'
		And VP.escort_snr='AE3089F5-9AC2-49F4-936D-F33BF54D80D2'
		) AS Vis_GteNac 
		...
		and MR.User_type in (4,5)
		and DM.User_type = 5
		and DM.user_snr not in ('E1A1BE94-6802-4106-848C-2BC9A36F6980','572E9158-DC3C-4960-A1FD-0834549170FB')
		...
		Order by DM.LNAME,DM.FNAME,MR.User_type,MR.LNAME,MR.FNAME,klr.REG_SNR ";
	...
	if($tipo == 1 || $tipo == 2){//excel
		...
		$pdf=new FPDF('L', 'mm', array(1100,1000));
	
		$pdf->AddPage();
		$pdf->SetFont('Arial','B',10);
		$pdf->setDisplayMode(100, 'continuous');
		$pdf->Cell(40,5,utf8_encode('Reporte de Cobertura de Visita Medica'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
	...
	$rsMedicos = sqlsrv_query($conn, utf8_encode($qMedicos));
	if( $rsMedicos === false ) {
	...
	while($reg = sqlsrv_fetch_array($rsMedicos)){
		...
		if($i == 1){
			...
			if($tipo != 3){
				$tabla .= '<td '.$estilocabecera.' rowspan="2" width="300px">Ruta - Nombre</td>';
				$tabla .= '<td '.$estilocabecera.' colspan="3" width="150px" align="center">'.date("d/m", strtotime($fechaI)).' - '.date("d/m", strtotime('+6 day', strtotime($fechaI))).'<br>Semana 1</td>';
				$tabla .= '<td '.$estilocabecera.' colspan="3" width="150px" align="center">'.date("d/m", strtotime('+7 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+13 day', strtotime($fechaI))).'<br>Semana 2</td>';
				$tabla .= '<td '.$estilocabecera.' colspan="3" width="150px" align="center">'.date("d/m", strtotime('+14 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+20 day', strtotime($fechaI))).'<br>Semana 3</td>';
				$tabla .= '<td '.$estilocabecera.' colspan="3" width="150px" align="center">'.date("d/m", strtotime('+21 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+27 day', strtotime($fechaI))).'<br>Semana 4</td>';
				$tabla .= '<td '.$estilocabecera.' colspan="3" width="150px" align="center">'.date("d/m", strtotime('+28 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+34 day', strtotime($fechaI))).'<br>Semana 5</td>';
				$tabla .= '<td '.$estilocabecera.' colspan="3" width="150px" align="center">Cobertura Total vs Plan</td>';
				...
				$tabla .= '<td '.$estilocabecera.' width="50px" align="center">% Cob.</td>';
				...
				$tabla .= '<td '.$estilocabecera.' width="100px" align="center">Gte Capacitacion</td>';
				$tabla .= '<td '.$estilocabecera.' width="100px" align="center">Total Vis Acomp</td>';	
			}else{
				$pdf->Ln();			
				$pdf->Cell(150,10,'','LTR',0,'L',1);
				$pdf->Cell(75,10,date("d/m", strtotime($fechaI)).' - '.date("d/m", strtotime('+6 day', strtotime($fechaI))),'LTR',0,'C',1);
				$pdf->Cell(75,10,date("d/m", strtotime('+7 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+13 day', strtotime($fechaI))),'LTR',0,'C',1);
				$pdf->Cell(75,10,date("d/m", strtotime('+14 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+20 day', strtotime($fechaI))),'LTR',0,'C',1);
				$pdf->Cell(75,10,date("d/m", strtotime('+21 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+27 day', strtotime($fechaI))),'LTR',0,'C',1);
				$pdf->Cell(75,10,date("d/m", strtotime('+28 day', strtotime($fechaI))).' - '.date("d/m", strtotime('+34 day', strtotime($fechaI))),'LTR',0,'C',1);
				$pdf->Cell(75,10,'','LTR',0,'L',1);
				$pdf->Cell(300,10,'','LTR',0,'L',1);
				$pdf->Cell(150,10,'','LTR',0,'L',1);
				$pdf->Ln();
				$pdf->Cell(150,10,'Ruta - Nombre','LR',0,'L',1);
				$pdf->Cell(75,10,'Semana 1','LR',0,'C',1);
				$pdf->Cell(75,10,'Semana 2','LR',0,'C',1);
				$pdf->Cell(75,10,'Semana 3','LR',0,'C',1);
				$pdf->Cell(75,10,'Semana 4','LR',0,'C',1);
				$pdf->Cell(75,10,'Semana 5','LR',0,'C',1);
				$pdf->Cell(75,10,'Cobertura Total vs Plan','LRB',0,'C',1);
				$pdf->Cell(300,10,'Cobertura Total vs Universo','LRB',0,'C',1);
				$pdf->Cell(150,10,'Resumen de Visita Acompañada','LRB',0,'C',1);
				$pdf->Ln();			
				$pdf->Cell(150,10,'','LRB',0,'L',1);
				$pdf->Cell(25,10,'V.P.',1,0,'C',1);
				$pdf->Cell(25,10,'V.R.',1,0,'C',1);
				$pdf->Cell(25,10,'% Cob.',1,0,'C',1);
				$pdf->Cell(25,10,'V.P.',1,0,'C',1);
				$pdf->Cell(25,10,'V.R.',1,0,'C',1);
				$pdf->Cell(25,10,'% Cob.',1,0,'C',1);
				$pdf->Cell(25,10,'V.P.',1,0,'C',1);
			...
			if($tempGerente == $gerente){
				...
				if($tipo != 3){					
					...
					$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($cobTotalGte, 2).' %</td>';
	...
	////imprimir nacional
	if($tipo != 3){
		$tabla .= '<tr><td '.$estilogte.' width="300px">Total General</td>';
		...
		$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($totalCob1, 2).' %</td>';
		...
		$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($totalCob2, 2).' %</td>';
		...
		$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($totalCob3, 2).' %</td>';
		...
		$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($totalCob4, 2).' %</td>';
		...
		$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($totalCob5, 2).' %</td>';
		...
		$tabla .= '<td '.$estilogte.' width="50px" align="right">'.number_format($cobTotalNal, 2).' %</td>';
