<?php
	...
	$tam = array(350,150,350,100,100,100,100,350,600,150);//,200,150,100,100,100,100,100,100, 150,150,100,100,350,350,150,100,100,350, 350,150,100,100,350,350,150,100,100,350, 350,150,100,100,350,350);//,100,100); //,100,100,50,100,100,100,100,100,100,50,100,100,100,50,100,150,100,100,100,100,100,150,100,100,100,150,100,100,100,50,100,50,100,50,100,50,100,50,100,50,100,100,100,100);
	...
	$qMedicos = "select distinct 
		...
		CODELIST.name as Actividad,
	    (select sum(value) from day_report_code where day_report_code.dayreport_snr=dr.dayreport_snr and day_report_code.rec_stat=0) as 'Total Horas',
		'' as 'Porc Dia',
		DC.value as 'Hrs. Act.',
		'' as 'Porc Act.',
		(select UC.LNAME+' '+UC.FNAME from USERS UC where UC.user_snr=DR.creator_user_snr ) as 'Creado por',
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoOtrasActividades".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado Otras Actividades");
	}
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO OTRAS ACTIVIDADES')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoOtrasActividades');
