<?php
	...
	$tam = array(100,350,350,150,150,200,250,150,600,600, 600,600,600,600,600); 
	...
	$qMedicos = "DECLARE @Encuesta as VARCHAR(36)
		...
		(CASE WHEN Resp1.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA1.name ELSE Resp1.ANSWER_STRING END) as '¿QUE TAN A MENUDO RECOMIENDA EL USO DE ENJUAGUES BUCALES A SUS PACIENTES?', 
		(CASE WHEN Resp2.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA2.name ELSE Resp2.ANSWER_STRING END) as '¿QUE TIPO DE ENJUAGUE BUCAL ES EL QUE MAS RECOMIENDA?', 
		(CASE WHEN Resp3.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA3.name ELSE Resp3.ANSWER_STRING END) as '¿CUALES SON LAS MARCAS DE ENJUAGUES BUCALES QUE MAS RECOMIENDA?', 
		(CASE WHEN Resp4.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA4.name ELSE Resp4.ANSWER_STRING END) as '¿CONOCE LOS ENJUAGUES BUCALES DE LA MARCA THERABREATH?', 
		(CASE WHEN Resp5.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA5.name ELSE Resp5.ANSWER_STRING END) as 'CUANDO RECOMIENDO UN ENJUAGUE BUCAL, NORMALMENTE RECOMIENDO:', 
		(CASE WHEN Resp6.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA6.name ELSE Resp6.ANSWER_STRING END) as 'COMO CONSIDERA EL USO DE ENJUAGUES BUCALES ES:', 
		(CASE WHEN Resp7.ANSWER_SNR<>'00000000-0000-0000-0000-000000000000' THEN SA7.name ELSE Resp7.ANSWER_STRING END) as 'CUAL ES LA RAZON PRINCIPAL POR LA QUE RECOMIENDA UN ENJUAGUE BUCAL' 
		...
		group by cl.name,U.lname,U.fname,P.pers_snr,P.lname,P.mothers_lname,P.fname,S.name,SAnsw.date,Resp1.ANSWER_SNR,Resp2.ANSWER_SNR,Resp3.ANSWER_SNR,Resp4.ANSWER_SNR,Resp5.ANSWER_SNR,Resp6.ANSWER_SNR,Resp7.ANSWER_SNR
		,SA1.name,SA2.name,SA3.name,SA4.name,SA5.name,SA6.name,SA7.name,Resp1.ANSWER_STRING,Resp2.ANSWER_STRING,Resp3.ANSWER_STRING,Resp4.ANSWER_STRING,Resp5.ANSWER_STRING,Resp6.ANSWER_STRING,Resp7.ANSWER_STRING 
