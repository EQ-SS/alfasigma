							...
							if($tipoUsuario == 2){
								echo '<div class="form-group display-flex margin-0 pull-right" style="width:50%;">
									...
									<select id="sltGte" class="form-control">
										...
										$queryGte = "select * from users where USER_TYPE = '5' order by lname";
										$rsGte = sqlsrv_query($conn, $queryGte);
										while($regGte = sqlsrv_fetch_array($rsGte)){
											$nameDividido = explode('-',$regGte["LNAME"]);
											$nameSinNR = count($nameDividido) >1 ? $nameDividido[1] : $nameDividido[0];
											$regGte["LNAME"] = $regGte["USER_NR"].' - '.$nameSinNR;
											echo '<option value="'.$regGte["USER_SNR"].'">'.$regGte["LNAME"].' '.$regGte["FNAME"].'</option>';				
										}
								echo '</select></div>';
							}
								...
								while($regRutas = sqlsrv_fetch_array($rsRutas)){
									$nameDividido = explode('-',$regRutas["LNAME"]);
									$nameSinNR = count($nameDividido) >1 ? $nameDividido[1] : $nameDividido[0];
									$regRutas["LNAME"] = $regRutas["USER_NR"].' - '.$nameSinNR;
									echo '<option value="'.$regRutas["USER_SNR"].'">'.$regRutas["LNAME"].' '.$regRutas["FNAME"].'</option>';				
								}
								echo '</select></div>
								</div>';
						}
										...
										<ul class="header-dropdown m-r--5">
											<li class="dropdown">
												...
												<ul class="dropdown-menu pull-right">
													<li id='exportpng3'><a href="javascript:void(0);">Descargar como PNG</a></li>
													<li id='exportpdf3'><a href="javascript:void(0);">Descargar como PDF</a></li>
												</ul>
											</li>
										</ul>
									</div>
									...
									<div class="header">
										...
										<ul class="header-dropdown m-r--5">
											<li class="dropdown">
												...
												<ul class="dropdown-menu pull-right">
													<li id='exportpng4'><a>Descargar como PNG</a></li>
													<li id='exportpdf4'><a>Descargar como PDF</a></li>
												</ul>
											</li>
										</ul>
									</div>
					...
					<div class="row">
						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<div class="card" style="height:550px;">
								<div class="header">
									<h2>Distribución de Frecuencia por Categoría</h2>
									<ul class="header-dropdown m-r--5">
										<li class="dropdown">
											<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
											 aria-expanded="false">
