<?php
	...
	$tam = array(100,300,250,250,100,100,100,150,200,200, 200,550,100,150,300,100,200,200,100,100, 100,100,200,200,100,100,100,100,150,150, 250,500,150,150,100,150,150,150,150,300, 120,120,120,120,550,550,100,100,100,100 ,100,100);//,100,100,100,100);//,50 ,100,100,100,100,100,100,50,100,100,100,50,100,150,100,100,100,100,100,150,100,100,100,150,100,100,100,50,100,50,100,50,100,50,100,50,100,50,100,100,100,100);
	...
	$qMedicos = "Select DISTINCT
		...
		,isnull((select cast(VPROD.POSITION as varchar(2)) from VISITPERS_PROD VPROD
		where VP.VISPERS_SNR=VPROD.VISPERS_SNR and VPROD.REC_STAT=0
		and VPROD.PROD_SNR='5161ED0C-74ED-400D-89BB-0C2DF1ADA4E0' ) 
		,'-') as ATEKA
		,isnull((select cast(VPROD.POSITION as varchar(2)) from VISITPERS_PROD VPROD
		where VP.VISPERS_SNR=VPROD.VISPERS_SNR and VPROD.REC_STAT=0
		and VPROD.PROD_SNR='4F897887-3A6A-4694-84F0-4232E4805F90' ) 
		,'-') as ESOXX
		,isnull((select cast(VPROD.POSITION as varchar(2)) from VISITPERS_PROD VPROD
		where VP.VISPERS_SNR=VPROD.VISPERS_SNR and VPROD.REC_STAT=0
		and VPROD.PROD_SNR='E21BB406-A8E6-47A9-AFBF-A79644835B3D' ) 
		,'-') as FLONORM
		,isnull((select cast(VPROD.POSITION as varchar(2)) from VISITPERS_PROD VPROD
		where VP.VISPERS_SNR=VPROD.VISPERS_SNR and VPROD.REC_STAT=0
		and VPROD.PROD_SNR='65C7A084-B2EB-4331-9030-9AA10B45F3D1' ) 
		,'-') as 'VESSEL DUE F'
		,isnull((select cast(VPROD.POSITION as varchar(2)) from VISITPERS_PROD VPROD
		where VP.VISPERS_SNR=VPROD.VISPERS_SNR and VPROD.REC_STAT=0
		and VPROD.PROD_SNR='E7CB6382-CBA0-4AB4-A1E6-7D5BAA27E8A8' ) 
		,'-') as 'ZIR COMBIE'
		,isnull((select cast(VPROD.POSITION as varchar(2)) from VISITPERS_PROD VPROD
		where VP.VISPERS_SNR=VPROD.VISPERS_SNR and VPROD.REC_STAT=0
		and VPROD.PROD_SNR='89FB9A5F-1721-4CF6-A029-8F1BA3D27028' ) 
		,'-') as ZIRFOS
		...
		/*and VP.visit_date between '2019-09-20' and '2019-09-23'*/ ";
