<?php
	...
	$tam = array(100,350,350,150,200,250,450,550,450,100, 250,250,150,150,250,200,250,100,150,150, 150,150,150,150,150,150,200,200);
	...
	$query = "Select
		...
		PERFIL1.name as Nivel_Atencion,
		PERFIL2.name as Tipo_Receta,
		PERFIL3.name as Surtido_Receta,
		PERFIL4.name as Tipo_cuadro_basico,
		PERFIL5.name as 'Productos AW en CB',
		...
		left outer join inst_profile as PER on I.inst_snr = PER.inst_snr		 
		left outer join inst_profile_ud as PERFIL on PER.instprofile_snr = PERFIL.instprofile_snr
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoHospitales".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado de Hospitales");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoHospitales.xls");
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO DE HOSPITALES'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO DE HOSPITALES</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO DE HOSPITALES')
