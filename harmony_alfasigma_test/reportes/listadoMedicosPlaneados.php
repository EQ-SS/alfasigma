<?php
	...
	$tam = array(100,350,350,350,150,150,350,450,200,150, 450,100,200,200,150,150,100,100,250,250, 150,150,150,100,200,200,200,200,100,100, 150,150,150,800);
	...
	$qMedicos = "Select
		...
		P.tel2 as Tel2,
		P.mobile as Celular,
		TipoPlan.name as Tipo_Plan,
		cast(cast(VP.PLAN_DATE as DATE) as nvarchar(10)) as Fecha_Plan,
		cast(VP.time as nvarchar(10)) as Hora_Plan,
		(case when visitpers.visit_date is null then 'Planeado no visitado' else 'Visitado' end) as Visitado,
		/*cast(cast(visitpers.visit_date as DATE) as varchar(10)) as Fecha_visita,*/
		...
		inner join pers_srep_work PSW on PLW.pwork_snr = PSW.pwork_snr 
		...
		left outer join visitpers on visitpers.vispers_snr = VP.vispers_snr
		left outer join codelist TipoPlan on VP.plan_code_snr = TipoPlan.clist_snr
		 
		left outer join person_ud PUD on PUD.pers_snr = P.pers_snr and PUD.rec_stat=0
		left outer join codelist Estilo_desc on PUD.field_01_snr = Estilo_desc.clist_snr and Estilo_desc.rec_stat=0
