<?php
	...
	$tam = array(350,200,300,250,300,100,100,100);
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ReporteInventarioCiclo".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Reporte de Inventario por Ciclo");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=reporteInventarioCiclo.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('REPORTE DE INVENTARIO POR CICLO'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">REPORTE DE INVENTARIO POR CICLO</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'REPORTE DE INVENTARIO POR CICLO')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ReporteInventarioCiclo');
