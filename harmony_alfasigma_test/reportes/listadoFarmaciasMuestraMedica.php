<?php
	...
	$tam = array(100,350,350,150,200,250,450,550,250,450, 100,250,250,150,100,350,350,300,100,150, 150,150,150,150,150);
	...
	$query = "Select
		...
		(case when I.rec_stat = 0 then '' else 'BAJA' end) as Inst_Baja,
		...
		left outer join codelist TipoVis on TipoVis.clist_snr = VP.visit_code_snr and TipoVis.rec_stat=0 
		left outer join codelist VisAcomp on cast(VisAcomp.clist_snr as varchar(36)) = cast(VP.escort_snr as varchar(36)) and VisAcomp.rec_stat=0 
		left outer join codelist Bonific on Bonific.clist_snr = VP.SUBJSTRAT_SNR and Bonific.rec_stat=0 
		left outer join codelist Importe on Importe.clist_snr = VP.KPIS_SNR and Importe.rec_stat=0 
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoFarmaciasVisitadasMM".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado de Farmacias Visitadas con Muestra Medica");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoFarmaciasVisitadasMM.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO DE FARMACIAS VISITADAS CON MUESTRA MEDICA'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO DE FARMACIAS VISITADAS CON MUESTRA MEDICA</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO DE FARMACIAS VISITADAS CON MUESTRA MEDICA')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoFarmaciasVisitadasMM');
