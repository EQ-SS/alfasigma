<?php
	...
	$tam = array(350,200,300,250,300,100,100,100,100);
	...
	$qMedicos = "Select
		...
		PBATCH.name as Lote,
		PBATCH.name_customer as Lote_Almacen, 
		/*C.name as CICLO, */
		isnull(sum(SPU.quantity),0) Entrada, 
		 
		isnull((select sum(SPU1.quantity) * -1 from STOCK_PRODFORM_USER SPU1
		where SPU1.PRODFBATCH_SNR = SPU.PRODFBATCH_SNR
		and SPU1.user_snr = SPU.user_snr
		and SPU1.rec_stat=0
		and SPU1.accepted=1
		and SPU1.table_nr=374
		and SPU1.quantity<0
		),0) as Transfer, 
		...
		inner join PRODFORMBATCH PBATCH on PBATCH.PRODFBATCH_SNR = SPU.PRODFBATCH_SNR /*and year(PBATCH.expiration_date) >= year(getdate()) -1 */
		inner join PRODFORM PF on PF.PRODFORM_SNR = PBATCH.PRODFORM_SNR 
		inner join PRODUCT P on P.PROD_SNR = PF.PROD_SNR 
		inner join USERS U on U.user_snr = SPU.user_snr
		inner join CYCLES C on C.cycle_snr = SPU.cycle_snr and C.rec_stat=0 and C.year >= year(getdate()) -1
		 
		where SPU.rec_stat=0
		and SPU.accepted=1
		and SPU.table_nr=374
		and SPU.quantity>0
		and PBATCH.rec_stat=0
		and PF.rec_stat=0
		and P.rec_stat=0
		and U.rec_stat=0
		...
		group by 
		U.lname, U.fname, U.user_snr
		,P.name, PF.name
		,P.PROD_SNR,PF.PRODFORM_SNR,PBATCH.PRODFBATCH_SNR
		,P.name,PF.name,PBATCH.name,PBATCH.name_customer,PBATCH.expiration_date
		,SPU.PRODFBATCH_SNR,SPU.user_snr
		/*C.name, C.cycle_snr, */
		 
		order by U.lname,U.fname,P.name,PF.name,PBATCH.name /*,C.name*/ ";
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ReporteInventario".date("dmYHis").".xlsx";
	...
	if($tipo == 1){//excel con formato
		...
		header("Content-Disposition: filename=reporteInventario.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('REPORTE DE INVENTARIO'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">REPORTE DE INVENTARIO</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'REPORTE DE INVENTARIO')
	...
	while($regMedico = sqlsrv_fetch_array($rsMedicos)){
		...
		for($j=0;$j<sqlsrv_num_fields($rsMedicos);$j++){
			...
			if($j == 8){
				$regMedico[$j] = $regMedico[5] - ($regMedico[6] + $regMedico[7]);
			}
