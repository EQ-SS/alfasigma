<?php
	...
	$tam = array(350,150,450,100,150,150,150,350,850,100, 150);
	...
	$qMedicos = "select distinct 
		/*Ugte.lname+' '+Ugte.fname as Gerente,*/
		U.lname+' '+U.fname as Representante,
		cast(cast(DR.date as DATE) as varchar(10)) as Fecha,
		act.name as Actividad,
	    (select sum(value) from day_report_code where DC.dayreport_snr = day_report_code.dayreport_snr and rec_stat=0) as 'Total Horas',
		'' as 'Porc Dia',
		(case when DC.value=0 then (select sum(value) from day_report_code where DC.dayreport_snr = day_report_code.dayreport_snr and rec_stat=0) else DC.value end) as 'Hrs. Actividad',
		'' as 'Porc Act',
		(select UC.lname+' '+UC.fname from users UC where UC.user_snr = DR.creator_user_snr ) as 'Creado por',
		DR.info as Comentarios,
		U.user_nr as Ruta_Repre,
