<?php
	...
	$tam = array(100,350,350,350,100,100,150,150,250,250, 250,550,250,150,450,100,200,200,150,150, 100,100,200,200,150,150,100,150,200,200, 250,400,150,150,100,150,150,150,250,350, 150,150,150,150,700,700);
	...
	$qMedicos = "Select DISTINCT
		...
		/*'' as Div_Med_Int,*/
		Estilo_desc.name as 'Estilo disc', 
		P.tel1 as Tel1,
		P.tel2 as Tel2,
		P.mobile as Celular,
		...
		case when VP.CHANGED_TIMESTAMP = '1900-01-01' then '' else cast(cast(VP.CHANGED_TIMESTAMP as DATE) as nvarchar(10)) end as 'Fecha Modificación',
		CONVERT(nvarchar(16), VP.SYNC_TIMESTAMP, 120) as 'Fecha Sincronización',
		...
		left outer join codelist PERSTYPE ON P.perstype_snr=PERSTYPE.CLIST_SNR
		...
		left outer join codelist Estilo_desc on PUD.field_01_snr = Estilo_desc.clist_snr and Estilo_desc.REC_STAT=0 and Estilo_desc.status=1
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoMedicosVisitadosHistorico".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado de Medicos Visitados Historico");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoMedicosVisitadosHistorico.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO DE MÉDICOS VISITADOS HISTORICO'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO DE MÉDICOS VISITADOS HISTORICO</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO DE MÉDICOS VISITADOS HISTORICO')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoMedicosVisitadosHistorico');
