<?php
	...
	$tam = array(350,200,200,600,150,100,150,100,150,100, 200,200,800,350,200,200,200,200,150,100, 100,100);  
	...
	$qMedicos = "select 
		...
		Tipo.name as Tipo_Evento,
		upper(Ev.place) as Lugar,
		upper(Ev.name) as Nombre_evento,
		cast(convert(date, Ev.START_DATE, 20) as nvarchar(10)) as Fecha_Inicial,
		convert(char(5), Ev.START_TIME, 108) as Hora_Inicial,
		cast(convert(date, Ev.FINISH_DATE, 20) as nvarchar(10)) as Fecha_Final,
		convert(char(5), Ev.FINISH_TIME, 108) as Hora_Final,
		Tipo_part.name as Tipo_Particip,
		Ev.ATTENDED_QUANTITY as No_Part,
		EspEv.name as Especialidad,
		Grupo_ter.name as Grupo_Terapeutico,
		upper(Ev.info) as Comentario,
		...
		isnull(Esp.name,'') as Esp_medico,
		convert(varchar(16), EvPers.CREATION_TIMESTAMP, 120) as Fecha_Asig_med,
		(case EvPers.attended when '0' then 'NO' when '1' then 'SI' end) as Asistencia,
		CatAudit.name as Categ_Audit,
		CatAS.name as Categ_AS
		 
		from EVENT Ev
		inner join Event_User EvUser on EvUser.event_snr = Ev.event_snr
		inner join Users U on U.user_snr = EvUser.user_snr
		left outer join CODELIST Tipo on Tipo.clist_snr = Ev.type_snr 
		left outer join CODELIST Tipo_part on Tipo_part.clist_snr = Ev.part_type_snr 
		left outer join CODELIST EspEv on EspEv.clist_snr = Ev.specialty_snr 
		left outer join CODELIST Grupo_ter on Grupo_ter.clist_snr = Ev.thgroup_snr 
		left outer join EVENT_PERS EvPers on EvPers.event_snr = Ev.event_snr and EvPers.user_snr = U.user_snr and EvPers.rec_stat=0
		left outer join PERSON P on P.pers_snr = EvPers.pers_snr
		...
		and U.status=1
		and U.rec_stat=0
		and Ev.rec_stat in (0,3)
		and EvUser.rec_stat in (0,3)
		and U.user_snr in ('".$ids."')
		and convert(date,Ev.START_DATE,101) between '".$fechaI."' and '".$fechaF."'
		/* and U.user_snr in ('2B6B9502-91B9-42FA-A337-4D98478483BE')
		and convert(date,Ev.START_DATE,101) between '2021-08-01' and '2021-08-30' */
		 
		order by U.lname,U.fname,Ev.START_DATE desc,Ev.name,P.lname,P.mothers_lname,P.fname ";
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoEventos".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();

		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado de Eventos");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoEventos.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO DE EVENTOS'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="3" class="nombreReporte">LISTADO DE EVENTOS</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO DE EVENTOS')
