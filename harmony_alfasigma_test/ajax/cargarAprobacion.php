<?php
	...
	if($tipo == 'p'){
		...
		CASE WHEN CF.NAME = 'NAME' THEN CFT.LONG_TEXT ELSE CF.NAME END AS NAME, 
		CFT.LONG_TEXT, 
		CASE WHEN CF.NAME = 'I_STREET1' THEN 17 WHEN CF.NAME = 'I_NUM_EXT' THEN 18 ELSE SHORT_TEXT END AS SHORT_TEXT 
		FROM CONFIG_FIELD_TRANSLATION CFT
		...
		AND CFT.LANG_NR = 8
		AND CFT.SORT_NUM > 0
		AND CF.NAME IN ('I_STREET1','I_NUM_EXT') 
		ORDER BY ORDEN";
		...
		$cod_pers = getCodPers($conn, $pers_snr);
		...
		//	echo $queryAppChage."<br><br>";
			...
			if(sqlsrv_num_rows($rsAppChange) == 0){//entro por android
					...
					if(array_key_exists($arrayNombres[$m], $arrPersApproval)){
						if($arrPerson[$arrayNombres[$m]] != $arrPersApproval[$arrayNombres[$m]]){
							if( is_object($arrPerson[$arrayNombres[$m]]) || is_object($arrPersApproval[$arrayNombres[$m]])){
								...
								echo "<script>$('#tblAprobacion').append('<tr><td><b>".$arrayValores[$m]."</b></td><td>".$fechaPers."</td><td><font color=\"#3F51B5\">".$fechaApp."</font></td></tr>');</script>";
							}else{
								echo is_object($arrPerson[$arrayNombres[$m]])." - ".is_object($arrPersApproval[$arrayNombres[$m]]);
								echo "<script>$('#tblAprobacion').append('<tr><td><b>".$arrayValores[$m]."</b></td><td>".$arrPerson[$arrayNombres[$m]]."</td><td><font color=\"#3F51B5\">".$arrPersApproval[$arrayNombres[$m]]."</font></td></tr>');</script>";
							}
						}
					}
				}
				...
				while($regAppChange = sqlsrv_fetch_array($rsAppChange)){
					if(strtoupper($regAppChange['NAME']) != 'NAME' && strtoupper($regAppChange['NAME']) != 'COLONIA' && strtoupper($regAppChange['NAME']) != 'ZIP' && strtoupper($regAppChange['NAME']) != 'DELEG/MPIO' && strtoupper($regAppChange['NAME']) != 'ESTADO'){
						...
						$indice = array_search(strtoupper($regAppChange['NAME']), $arrayNombres);
					...
					if($indice > -1){
						if(is_object($regPersApproval[$arrayNombres[$indice]])){
							...
							if($fechaApp != $fechaPers){
								echo "<script>$('#tblAprobacion').append('<tr><td><b>".$arrayValores[$indice]."</b></td><td>".$fechaPers."</td><td><font color=\"#3F51B5\">".$fechaApp."</font></td></tr>');</script>";
							}else{
							...
							if($regPersApproval[$arrayNombres[$indice]] != $regPers[$arrayNombres[$indice]]){
								echo "<script>$('#tblAprobacion').append('<tr><td><b>".$arrayValores[$indice]."</b></td><td>".$regPers[$arrayNombres[$indice]]."</td><td><font color=\"#3F51B5\">".$regPersApproval[$arrayNombres[$indice]]."</font></td></tr>');</script>";
							}else{
			...
			for($i=0;$i<count($arrayNombres);$i++){
				if(is_object($regPersApproval[$arrayNombres[$i]])){
					...
					echo "$('#tblAprobacion').append('<tr><td width=\"50%\"><b>".$arrayValores[$i]."</b></td><td>".strtoupper(str_replace("\r\n", " ",$regPersApproval[$arrayNombres[$i]]))."</td></tr>');";
				}
			}
			echo "</script>";
			...
			honorarios.name as P_FEE_TYPE_SNR,
			p.P_BIRTHDATE,
			categ.name as CATEGORY_NR,
			p.P_PROF_ID AS P_PROF_ID,
			freq.name as P_FRECVIS_SNR,
			P_DIFFVIS_SNR as P_DIFFVIS_SNR,		
			/*p.P_ADRESA2_BANKE,*/
			/*botiquin.name as P_RX_TYPE_SNR,*/
			P_EMAIL1,
			P_TEL1,
			/*P_ABIERTO1,
			P_ABIERTO2,
			...
			left outer join DISTRICT dis on dis.DISTR_SNR = CITY.DISTR_SNR
			left outer join STATE edo on edo.STATE_SNR = city.STATE_SNR
			left outer join CODELIST motivo on motivo.CLIST_SNR = p.PLW_DEL_STATUS_SNR 
			left outer join CODELIST tipo on tipo.CLIST_SNR = p.P_PERSTYPE_SNR 
			where pers_approval_snr = '".$id."'";
			...
			echo "$('#tblAprobacion').append('<tr><td width=\"50%\"><b>Motivo de la baja</b></td><td>".$regPersApproval['motivo']."</td></tr>');";
			echo "$('#tblAprobacion').append('<tr><td width=\"50%\"><b>Notas adicionales</b></td><td>".str_replace("\n", " ",$regPersApproval['comentariosBaja'])."</td></tr>');";
			echo "</script>";
		}
	}else if($tipo == 'i'){
		...
		if($approvalStatus == 'C'){
			...
			$rsAppChange = sqlsrv_query($conn, $queryAppChage);
			$domicilio = true;
			while($regAppChange = sqlsrv_fetch_array($rsAppChange)){
			...
			if($approvalStatus == 'D'){
				echo "$('#tblAprobacion').append('<tr><td width=\"50%\"><b>Motivo de la baja</b></td><td>".$regInstApproval['motivo']."</td></tr>');";
