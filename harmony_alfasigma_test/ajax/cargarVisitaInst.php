<?php
	...
	if(isset($_POST['idVisita']) && $_POST['idVisita'] != '' && $_POST['idVisita'] != '00000000-0000-0000-0000-000000000000'){
		...
		$queryRepre = sqlsrv_fetch_array(sqlsrv_query($conn, "select user_nr as ruta, lname + ' ' + mothers_lname + ' ' + fname as nombre from users where USER_SNR = '".$idUsuario."'"));
		...
		$query ="select i.name as nombre_inst, 
			...
			bri.name as brick,
			i.inst_type as tipoInst,
			(select info_nextvisit from VISITINST where INST_SNR = i.inst_snr and VISIT_DATE in (select max(VISIT_DATE) from VISITINST where inst_snr = i.inst_snr)) as info_ult_vis
	...
	if($idVisita == ''){
		...
		$firma = "";
		$infoUltimoPlan = "";
		$visitCode = "00000000-0000-0000-0000-000000000000";
		$visitAcom = array("BF5B7926-CB6D-409D-8B52-081D1FCC732C");
		$infoSigVisita = "";
		$rep = $queryRepre['nombre'];
		$ruta = $queryRepre['ruta'];
		//$idPers = $visita['pers_snr'];
		//$persona = $visita['nombre'];
		//$especialidad = $visita['especialidad'];
		$dir = $visita['direccion']." ".strtoupper('Calle: '.$visita['direccion'].', C.P.: '.$visita['cpostal'].', Colonia: '.$visita['colonia'].', Población: '.$visita['pob'].', Estado: '.$visita['edo'].'.');
		$brick = $visita['brick'];
		...
		while($visita = sqlsrv_fetch_array($rsVisita)){
			if($contador == 0){
				...
				if(substr($visita['visAcomp'],0,-1) == ";"){
					$visitAcom = explode(";", substr($visita['visAcomp'],0,-1));
				}else{
	...
	echo "<script>
		...
		$('#lblInstVisitasInst').text('".$inst."');
		$('#lblDirVisitasInst').text('".$dir."');
		...
		$('#txtComentariosVisitaInst').val('".$infoVisita."');
		$('#txtInfoSiguienteVisitaInst').val('".$infoSigVisita."');
		...
		/*$rsCodigo = llenaComboInst($conn, 33, 69, );
		far 34,1
		hos 34,2*/
		if($tipoInst == 2){
		...
		//$('#lstCodigoVisitaInst').val('".$visitCode."');
		echo "$('#lstCodigoVisitaInst').empty();";
		while($codigo = sqlsrv_fetch_array($rsCodigo)){
			if($visitCode == $codigo['id']){
				echo "$('#lstCodigoVisitaInst').append('<option value=\"".$codigo['id']."\" selected >".$codigo['nombre']."</option>');";
			}else{
				echo "$('#lstCodigoVisitaInst').append('<option value=\"".$codigo['id']."\" >".$codigo['nombre']."</option>');";
			}
		}
	...
	if($idVisita == ''){
		...
		if($tipoUsuario != 4){
			...
			$rep = $regUser['USER_NR'].' - '.$regUser['LNAME'].' '.$regUser['FNAME'].' '.$regUser['MOTHERS_LNAME'];

			$nameDividido = explode('-',$rep);
			$nameSinNR = count($nameDividido) >2 ? $nameDividido[0].' - '.$nameDividido[2] : $nameDividido[0].' - '.$nameDividido[1];
			$rep = $nameSinNR;
			echo "$('#sltRepreVisitasInst').append('<option value=\"".$regUser['USER_SNR']."\" selected=\"selected\">".$rep."</option>');";
		}
		$queryProductos = "select * from product where REC_STAT = 0 order by name";
		$rsProdcutos = sqlsrv_query($conn, $queryProductos);
		$renglon = '<tr class="align-center"><td style="vertical-align:middle;" class="col-red">1</td><td><div class="select"><select id="lstProductoInst1" onchange="llenaSiguienteVisitasInst(1);" class="form-control">';
		...
		//echo "select * from codelist where CLIb_SNR = (select  clib_snr from codelistlib where table_nr = 33 and clib_type = 142) and status = 1 order by name";
		$check = 1;
		$contadorChecks = 0;
		$descripcionesCheck = '';
		//print_r($visitAcom);
		...
		//echo "alert('".$descripcionesCheck."');";
		echo "$('#hdnTotalChecksInst').val('".$contadorChecks."');
			$('#hdnDescripcionChkVisitasInst').val('".$descripcionesCheck."');
			$('#sltMultiSelectInstVisita').text('".$descripcionesCheck."');";
				
		/* fin visita acompañada*/
		$queryProductos = "select p.prod_snr as idProducto, p.name as producto, vp.POSITION as posicion, vp.PERCENTAJE as porcentaje
		...
		if(sqlsrv_num_rows($rsProdcutos)>0){
			...
			$queryProductos = "select * from product where REC_STAT = 0 order by name";
			$rsProdcutos = sqlsrv_query($conn, $queryProductos);
			$renglon = '<tr class="align-center"><td style="vertical-align:middle;" class="col-red">1</td><td><select id="lstProductoInst1" class="form-control" onchange="llenaSiguienteVisitasInst(1);">';
			while($producto = sqlsrv_fetch_array($rsProdcutos)){
				$renglon .= '<option value="'.$producto['PROD_SNR'].'">'.$producto['NAME'].'</option>';
			}
		...
		while($stock = sqlsrv_fetch_array($rsStock)){
			echo "$('#tblProductosStockConsulta tbody').append('<tr><td width=\"500px\" >".$stock['producto']." ".$stock['presentacion']."</td><td width=\"100px\" align=\"center\">".$stock['existencia']."</td><td width=\"100px\" align=\"center\">".$stock['precio']."</td><td width=\"100px\" align=\"center\">".$stock['pedido']."</td><td width=\"100px\" align=\"center\">".$stock['sugerido']."</td></tr>');";
		}
	}
	...
	echo "$('#liVisitaInst').click();
		...
		//$('#sltMultiSelectInstVisita').text('Selecciona');
		//$('#acompaInst1').attr('checked', false);
	</script>";
?>