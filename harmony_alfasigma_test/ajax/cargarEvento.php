<?php
	...
	if(! $conn){
		...
		$renglonSeleccionado = $_POST['renglonSeleccionado'];
		
		echo "<script>
				$('#tblInvitadosEditar tbody').empty();";
		$arrAsistentes = array();
		if($idEvento != ''){
			$qAsistentes = "select * from EVENT_PERS where rec_stat = 0 and event_snr = '".$idEvento."' ";
			$rsAsistentes = sqlsrv_query($conn, $qAsistentes,  array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
			
			while($asistente = sqlsrv_fetch_array($rsAsistentes)){
				$arrAsistentes[] = $asistente['PERS_SNR'];
			}
			
			$query = "select p.PERS_SNR, u.USER_NR as ruta, u.LNAME + ' ' + u.MOTHERS_LNAME + ' ' + u.FNAME as repre,
			...
			if(sqlsrv_num_rows($rsAsistentes)){
				$query .= "and psw.rec_stat = 0
					and ep.rec_stat = 0 ";
			}
			//echo $query;
			...
			while($evento = sqlsrv_fetch_array($rs)){
				if($reg == 0){
					...
					$especialidadEvento = $evento['SPECIALTY_SNR'];
					$grupoTerapeutico = $evento['THGROUP_SNR'];
					$comentarios = $evento['INFO'];
				}
				$nombre = $evento['nombre'];
				$especialidad = $evento['especialidad'];
				$dir = $evento['calle'].' CP. '.$evento['cp'].' COL. '.$evento['colonia'].', '.$evento['distrito'].', '.$evento['estado'];
				$categoria = $evento['categoria'];
				$idPersona = $evento['PERS_SNR'];
				if(in_array($idPersona, $arrAsistentes)){
			...
			$query = "select USER_NR as ruta, LNAME + ' ' + MOTHERS_LNAME + ' ' + FNAME as nombre
				from USERS 
				where user_snr = '".$idUser."'";
			
			$repre = sqlsrv_fetch_array(sqlsrv_query($conn, $query))['nombre'];
			$ruta = sqlsrv_fetch_array(sqlsrv_query($conn, $query))['ruta'];
		...
		echo "$('#hdnIdEventoEditar').val('".$idEvento."');
			...
			$('#hdnTrSeleccionadaEvento').val('".$renglonSeleccionado."');
			$('#tabInformacionEvento').click();
			</script>";
	}
?>