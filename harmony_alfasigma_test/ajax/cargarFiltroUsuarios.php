<?php
	...
	$queryUsuarios = "select * from users where user_snr in ('".$ids."') and user_type in (4,5)  and rec_stat=0";
	if($palabra != ''){
	...
	while($usuario = sqlsrv_fetch_array($rsUsuario)){
		$nombreRepre = $usuario['USER_NR'].' - '.$usuario['LNAME'].' '.$usuario['FNAME'];
		$nameDividido = explode('-',$nombreRepre);
		$nameSinNR = count($nameDividido) >2 ? $nameDividido[0].' - '.$nameDividido[2] : $nameDividido[0].' - '.$nameDividido[1];
		$nombreRepre = $nameSinNR;
		if($seleccionarTodo != ''){
