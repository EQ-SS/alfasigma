<?php
	...
	for($i=0;$i<count($arrIds);$i++){
		...
		$queryDesactivarRutaAnterior = "update pers_srep_work 
			set rec_stat = '2622', 
			sync = 0 
			where persrep_snr = '".$persrep_snr."' 
			and rec_stat = 0 ";
		
		$queryValidarPers_Srep_Work = "select * 
			from pers_srep_work 
			where user_snr = '".$rutaNueva."' 
			and inst_snr = '".$inst_snr."' 
			and pers_snr = '".$pers_snr."' 
			and pwork_snr = '".$pwork_snr."' ";
		...
		if($existeRegistroPers_Srep_Work > 0){////////existe
			$queryActivaRutaNueva = "update pers_srep_work 
				set rec_stat = '0', 
				sync = 0 
				where user_snr = '".$rutaNueva."' 
				and inst_snr = '".$inst_snr."' 
		...
		//valida que no tenga mas medicos con esa dirección de la ruta
		$queryCantidadMedicosMismaDireccion = "select * 
			from PERS_SREP_WORK 
			where USER_SNR='".$rutaAnterior."' 
			and INST_SNR='".$inst_snr."' ";
			
		$rsCantidadMedicosMismaDireccion = sqlsrv_query($conn, $queryCantidadMedicosMismaDireccion, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
		
		if(sqlsrv_num_rows($rsCantidadMedicosMismaDireccion) == 1){
			$queryDesativoUserTerritAnterior = "update user_territ 
				set rec_stat = 2622, 
				sync = 0 
				where inst_snr = '".$inst_snr."' 
				and user_snr = '".$rutaAnterior."' 
				and rec_stat = 0";
		...
		$queryValidarUser_territ = "select * 
			from user_territ 
			where user_snr = '".$rutaNueva."' 
			and inst_snr = '".$inst_snr."' ";
