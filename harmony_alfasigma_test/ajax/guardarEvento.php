<?php
	...
	if(! $conn){
		...
		$lugarEvento = utf8_decode(strtoupper($_POST['lugarEvento']));
		$nombreEvento = utf8_decode(strtoupper($_POST['nombreEvento']));
		...
		$comentarios = utf8_decode(strtoupper($_POST['comentarios']));
		$idUser = $_POST['idUser'];
		...
		if($renglonSeleccionado != ''){
			echo "traeAsistentesEventos('".$idEvento."','".$renglonSeleccionado."');";
		}else{
			echo "$('#imgEventos').click();";
		}
