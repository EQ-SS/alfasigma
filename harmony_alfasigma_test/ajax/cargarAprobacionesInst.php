<?php
	...
	if(! $conn){
		...
		$rs = sqlsrv_query($conn, $query, array(), array("Scrollable" => 'static'));
		$rs2 = sqlsrv_query($conn, $query, array(), array("Scrollable" => 'static'));
		
		$total_rows = sqlsrv_num_rows($rs);
		...
		echo "$('#numRegistrosAprobInst').text('Registros: ".$total_rows."');
		</script>";
	}
?>