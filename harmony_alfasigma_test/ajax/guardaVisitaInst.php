		...
		if(isset($_POST['pantalla']) && $_POST['pantalla'] == 'cal'){
			...
			$queryVisitas = "select visit_date as Fecha_Vis,
				...
				while($visita = sqlsrv_fetch_array($rsVisitas)){
					...
					echo "$('#tblVisitasInst tbody').append('<tr onClick=\"muestraVisitaInst(\'".$visita['visinst_snr']."\');\"><td style=\"width:11%;\">".$visita['ciclo']."</td><td style=\"width:8%;\">".$visita['user_nr']."</td><td style=\"width:13%;\">".$fecha_vis."</td><td style=\"width:9%;\">".$visita['time']."</td><td style=\"width:19%;\">".$visita['Tipo_Vis']."</td><td style=\"width:20%;\">".$visita['informacion_vis']."</td><td style=\"width:20%;\">".$visita['obj_vis']."</td></tr>');";
				}
				//echo $visitas;
				$totalVisitas = 0;
