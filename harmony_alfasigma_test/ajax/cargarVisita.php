<?php
	...
	if(! $conn){
		...
		if($idVisita == ''){
			...
			$minutos = date("i");
			$tipoVisita = "";
			$infoVisita = str_ireplace($buscar,$reemplazar,$visita['obj_sig_vis']);
			$visAcomp = "";
			$firma = "";
			$infoUltimoPlan = "";
			$visitCode = "01A87BCB-57BD-4180-A2CB-8DB4C464F6A3";
			...
			while($visita = sqlsrv_fetch_array($rsVisita)){
				if($contador == 0){
					...
					$infoVisita = str_ireplace($buscar,$reemplazar,$visita['informacion_vis']);
					$idPers = $visita['pers_snr'];
					$visAcomp = $visita['vis_acomp'];
					$firma = $visita['firma'];
					$infoUltimoPlan = $visita['info_Ult_Plan'];
					$persona = $visita['nombre'];
					$especialidad = $visita['especialidad'];
					$dir = $visita['inst']." ".strtoupper('Calle: '.$visita['calle'].', C.P.: '.$visita['cp'].', Colonia: '.$visita['col'].', Población: '.$visita['del'].', Estado: '.$visita['estado'].'.');
					$brick = $visita['brick'];
					...
					$infoSigVisita = str_ireplace($buscar,$reemplazar,$visita['INFO_NEXTVISIT']);
					$idUsuario = $visita['user_snr'];
					$enfermedades_atiende = $visita['enfermedades_atiende'];
					if($visita['producto'] != ''){
		...
		if($idVisita == ''){
			...
			if(sqlsrv_num_rows($rsProdcutos)>0){
				...
				$rsProdcutos = sqlsrv_query($conn, $queryProductos);
				$renglon = '<tr class="align-center"><td style="vertical-align:middle;" class="col-red">1</td><td><select id="lstProducto1" style="width:150px;" onchange="llenaSiguiente(1);">';
				while($producto = sqlsrv_fetch_array($rsProdcutos)){
		...
		if(count($productos)>0){
			...
			$numTexto = 1;
				...
				$queryPromociones = "select * from (
					...
					INNER JOIN PRODFORMBATCH LOTE ON LOTE.PRODFORM_SNR=PF.PRODFORM_SNR 
					where LOTE.EXPIRATION_DATE > GETDATE() ) a
					WHERE a.ENTRADA>0 
		...
		";
		if($idVisita == ''){
			echo "<script>	$('#sltMultiSelect').text('Selecciona');</script>";
		}
		
	}
	//echo "<br>".$queryPromociones."<br>";
	//echo $queryPromociones;
	//echo "estos es:".$queryPromociones;
	//$('#imgFirma').attr('src', 'data:image/png;base64,".str_replace(" ","+",str_ireplace($buscar,$reemplazar,$firma))."');
?>