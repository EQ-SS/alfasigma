<?php
	...
	if(! $conn){
		...
		while($evento = sqlsrv_fetch_array($rs)){
			...
			$nombreEvento = str_replace("'", "\'", $evento['nombre']);
			$tipoEvento = str_replace("'", "\'", $evento['tipo']);
			$fechaInicial = $evento['START_DATE'];
			$horaInicial = $evento['START_TIME'];
			$fechaFinal = $evento['FINISH_DATE'];
			$horaFinal = $evento['FINISH_TIME'];
			$lugar = str_replace("'", "\'", $evento['PLACE']);
			$comentarios = str_ireplace($buscar,$reemplazar,str_replace("'", "\'", $evento['INFO']));
