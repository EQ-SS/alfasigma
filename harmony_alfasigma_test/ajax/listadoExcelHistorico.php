<?php
    ...
    $numCol=$_POST['numCol'];
    $nombreListado="Listado_Historico_Medicos";
    ...
    $reemplazar=array(" ", " ", " ", " ");
    $tam = array(100,100,100,300,350,350,200,200,300,150, 100,250,100,150,150,100,100,100,100,100, 100,100,100,100,100,100,100,100,100,100 ,100,100,100,100,100,100,100,100,100,100 ,100,100,100,100,100,100,100,100,100,100 ,100,100,100,100,100,100,100,100,100,100 ,100,100,100,100,100);
    ...
    $tabla = '<table border="0">
        <tr>
            <td>
                <table>
                    <tr>
                        <td colspan="10" class="nombreReporte">LISTADO DE MEDICOS VISITADOS HISTORICO POR CICLO TOTAL</td>
                    </tr>
                    <tr>
    ...
    foreach(sqlsrv_field_metadata($rsMedicos) as $field){
        $celda = columna($i)."4";
        if($i <= $numCol){
            $tabla .= '<td style="min-width:'.$tam[$i].'px;">'.$field['Name'].'</td>';
            $k = $i;
        }else{
            if($i % 2 == 0){
                $cabeceras[] = $field['Name'];
            }
        }
        $i++;
    }
        ...
        if($i == 1){
            ...
            for($l = count($cabeceras)-1; $l >= 0; $l--){ 
                if ($regMedico[$cabeceras[$l]] != 'A'){
                    $tabla .= '<td style="min-width:'.$tam[$k].'px;">'.$regMedico[$cabeceras[$l]].'</td>';
                    $k++;					
                }
            }
            $tabla .= '</tr></thead>';
