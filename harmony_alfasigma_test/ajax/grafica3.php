...
$tipoRuta = "";
$idRuta = "";
//echo "<script>alert('__".$idUser."');</script>";
if(isset($_POST['idRuta']) && $_POST['idRuta'] != ''){
	...
	if($tipoRuta != 2 && $tipoRuta != 4){
		...
		//echo "___select KLOC_SNR from KLOC_REG where REG_SNR = '".$ids."' ";
		$rutas = array();
...
if(isset($_POST['tipoContacto']) && $_POST['tipoContacto'] != ''){
...
//echo "<br>cuota: ".$cuota."<br>";
//echo "<br>tipo de usu:".$tipoUsuario." ::: ".$tipoRuta;
/*if($tipoUsuario == 5){
	//echo "select count(*) as rep from kloc_reg where REG_SNR = '".$idUser."' and KLOC_SNR <> '".$idUser."' group by REG_SNR";
	if($tipoRuta != ''){
		$cuentaUsuarios = sqlsrv_fetch_array(sqlsrv_query($conn, "select count(*) as rep from kloc_reg where REG_SNR = '".$idRuta."' and KLOC_SNR <> '".$idRuta."' group by REG_SNR"))['rep'];
	}else{
