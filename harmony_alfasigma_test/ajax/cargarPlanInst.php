<?php
	...
	if(isset($_POST['idPlan']) && $_POST['idPlan'] != ''){
		...
		$query = "select vp.plan_date as fecha_plan, 
			...
			u.user_nr as ruta, u.lname+' '+u.fname as repre, 
			vp.visinst_snr,
		...
		$queryRepre = sqlsrv_fetch_array(sqlsrv_query($conn, "select user_nr as ruta, lname + ' ' + mothers_lname + ' ' + fname as nombre from users where USER_SNR = '".$idUser."'"));
	...
	if($idPlan == ''){
		...
		$ruta = $plan['ruta'] = $queryRepre['ruta'];
		if($tipoUsuario == 4){
			$repre = $idUser;
		}
		...
		$objetivoPlan =str_ireplace($buscar,$reemplazar,$plan['objetivo']);
		$idInst = $plan['inst_id'];
	...
	echo "<script>
		...
		$('#lblNombreInstPlanInst').text('".$plan['nombre_inst']."');
		$('#lblCallePlanInst').text('".strtoupper($plan['direccion'])."');
		$('#lblCPPlanInst').text('".$plan['cpostal']."');
		$('#lblColoniaPlanInst').text('".strtoupper($plan['colonia'])."');
		$('#lblPoblacionPlanInst').text('".strtoupper($plan['pob'])."');
		$('#lblEstadoPlanInst').text('".strtoupper($plan['edo'])."');
		$('#txtFechaPlanInst').val('".$fecha_plan."');
		$('#lstHoraPlanInst').val('".$hora."');
		$('#lstMinutosPlanInst').val('".$minutos."');
		$('#lstCodigoPlanInst').val('".$codigo_visita."');
		$('#objetivoPlanInst').val('".utf8_decode($objetivoPlan)."');
		$('#sltReprePlanInst').empty();
		";
	
		$nameDividido = explode('-',$repreNombre);
		$nameSinNR = count($nameDividido) >1 ? $nameDividido[1] : $nameDividido[0];
		$repreNombre= $ruta.' - '.$nameSinNR;
	if($tipoUsuario == 4){
		...
		if($idPlan == ''){
			...
			$nameDividido = explode('-',$queryRepre['nombre']);
			$nameSinNR = (count($nameDividido)) >1 ? $nameDividido[1] : $nameDividido[0];
			$queryRepre['nombre'] = $plan['ruta'].' - '.$nameSinNR;

			$dividido = explode('-',$regSuper['LNAME']);
			$nameSinNR = (count($dividido)) >1 ? $dividido[1] : $dividido[0];
