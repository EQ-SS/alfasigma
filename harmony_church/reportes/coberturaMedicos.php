<?php
	...
	$qMedicos = "DECLARE @DIAS_IN as DATE
		...
		/*
		SET @DIAS_IN='2023-05-06'
		SET @DIAS_FIN='2023-05-31'
		*/
		...
		and VP.visit_date Between @DIAS_IN and @DIAS_FIN
		and P.status_snr = @STATUS
		and substring(VP.escort_snr,1,36) = '4AFE5D70-7759-47B8-9333-E9CF289D4582'
		) as Vis_GteDto,
		...
		and VP.visit_date Between @DIAS_IN and @DIAS_FIN 
		and P.status_snr = @STATUS
		and substring(VP.escort_snr,1,36) = '4AD43186-EB43-4023-9120-AB82417ADC05'
		) as Vis_GteNac 
	...
	if($tipo == 1 || $tipo == 2){//excel
		...
		$pdf=new FPDF('L', 'mm', array(1100,1150));
	...
	while($reg = sqlsrv_fetch_array($rsMedicos)){
		...
		if($i == 1){
			...
			if($tipo != 3){
				...
				$tabla .= '<td '.$estilocabecera.' colspan="1" width="100px" align="center">Resumen de Visita Acompañada</td></tr>';
				...
				/*$tabla .= '<td '.$estilocabecera.' width="100px" align="center">Gte Nacional</td>';*/
				...
				$pdf->Cell(50,10,'Resumen de Visita ','LTR',0,'C',1);
				$pdf->Ln();
				...
				$pdf->Cell(50,10,'Acompañada','LRB',0,'C',1);
				$pdf->Ln();	
			...
			if($tempGerente == $gerente){
				...
				if($tipo != 3){					
					...
					$pdf->Cell(50,10,$gteVisAcompDto,1,1,'C',1);
		...
		if($tipo != 3){
			...
			$pdf->Cell(50,10,$reg['Vis_GteDto'],1,1,'C',0);
	...
	if($tipo != 3){
		...
		$pdf->Cell(50,10,$totalVisAcompDto,1,1,'C',1);
