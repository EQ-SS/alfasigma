<?php
	...
	$tam = array(100,350,350,100,200,250,450,550,450,100, 250,250,200,100,250,250,150,150);//,200,150, 150,150,150,150,150,150,150,100,100,100, 100,100,150,250,100,100,150,100,100,100, 100,100,100,500);//,100,100,100,100,100,500);//, 50,100,100,100,100,100,100,50,100,100,100,50,100,150,100,100,100,100,100,150,100,100,100,150,100,100,100,50,100,50,100,50,100,50,100,50,100,50,100,100,100,100);
	...
	$query = "Select
		...
		I.tel1 as Tel1,
		I.tel2 as Tel2,
		CAST(CAST(I.creation_timestamp as DATE) as VARCHAR(10)) as Fecha_Alta,
		CAST(CAST(I.changed_timestamp as DATE) as VARCHAR(10)) as Fecha_Mod_Baja
		...
		left outer join codelist T on T.clist_snr = I.type_snr
		left outer join codelist ST on ST.clist_snr = I.subtype_snr
		...
		and U.status=1
		and U.user_type=4
		and (I.inst_snr not in (select VI.inst_snr from visitinst VI where VI.visit_date between '".$fechaI."' and '".$fechaF."' and VI.rec_stat=0
		and U.user_snr=VI.user_snr))
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoFarmaciasNoVisitadas".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado de Farmacias No Visitadas");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoFarmaciasNoVisitadas.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO DE FARMACIAS NO VISITADAS'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO DE FARMACIAS NO VISITADAS</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO DE FARMACIAS NO VISITADAS')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoFarmaciasNoVisitadas');
