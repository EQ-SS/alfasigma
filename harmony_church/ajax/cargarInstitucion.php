<?php
	...
	if(! $conn){
		...
		$query = "select i.name, INST_TYPE.NAME as tipo, 
			i.STREET1 +' NUM. EXT. '+ i.num_ext + ' ' +city.name +' '+ d.NAME +' '+state.NAME as direccion,
			i.TEL1, i.TEL2, EMAIL1 as EMAIL, i.web as HTTP, i.INST_TYPE as idTipo,
			i.LATITUDE, i.LONGITUDE, i.FRECVIS_SNR,
			iud.field_01, field_01.name as field_01_snr, bri.name as brick
			from INST i
			...
			left outer join CODELIST field_01 on field_01.CLIST_SNR = iud.FIELD_01_SNR 
			where i.inst_snr = '".$id."'";
		...
		/*if($reg['FIELD_05'] != ""){
			if(substr($reg['FIELD_05'], -1) == ";"){
				$reg['FIELD_05'] = substr($reg['FIELD_05'], 0, -1);
			}
			$reg['FIELD_05'] = str_replace(";", "','", $reg['FIELD_05']);
			$qField_05 = "select name from codelist where clist_snr in ('".$reg['FIELD_05']."')";
			//echo "<br>".$qField_05."<br>";
			$rsField_05 = sqlsrv_query($conn, $qField_05);
			$reg['FIELD_05'] = "";
			while($field_05 = sqlsrv_fetch_array($rsField_05)){
				$reg['FIELD_05'] .= utf8_encode($field_05['name']).",";
			}
			$reg['FIELD_05'] = substr($reg['FIELD_05'], 0, -1);
		}*/
		
		/*if($reg['FIELD_01HOSP'] != ""){
			if(substr($reg['FIELD_01HOSP'], -1) == ";"){
				$reg['FIELD_01HOSP'] = substr($reg['FIELD_01HOSP'], 0, -1);
			}
			$reg['FIELD_01HOSP'] = str_replace(";", "','", $reg['FIELD_01HOSP']);
			$qField_01HOSP = "select name from codelist where clist_snr in ('".$reg['FIELD_01HOSP']."')";
			//echo "<br>".$qField_01HOSP."<br>";
			$rsFIELD_01HOSP = sqlsrv_query($conn, $qField_01HOSP);
			$reg['FIELD_01HOSP'] = "";
			while($FIELD_01HOSP = sqlsrv_fetch_array($rsFIELD_01HOSP)){
				$reg['FIELD_01HOSP'] .= utf8_encode($FIELD_01HOSP['name']).",";
			}
			$reg['FIELD_01HOSP'] = substr($reg['FIELD_01HOSP'], 0, -1);
		}*/
		
		/*if($reg['FIELD_08HOSP'] != ""){
			if(substr($reg['FIELD_08HOSP'], -1) == ";"){
				$reg['FIELD_08HOSP'] = substr($reg['FIELD_08HOSP'], 0, -1);
			}
			$reg['FIELD_08HOSP'] = str_replace(";", "','", $reg['FIELD_08HOSP']);
			$qField_08HOSP = "select name from codelist where clist_snr in ('".$reg['FIELD_08HOSP']."')";
			//echo "<br>".$qField_01HOSP."<br>";
			$rsFIELD_08HOSP = sqlsrv_query($conn, $qField_08HOSP);
			$reg['FIELD_08HOSP'] = "";
			while($FIELD_08HOSP = sqlsrv_fetch_array($rsFIELD_08HOSP)){
				$reg['FIELD_08HOSP'] .= utf8_encode($FIELD_08HOSP['name']).",";
			}
			$reg['FIELD_08HOSP'] = substr($reg['FIELD_08HOSP'], 0, -1);
		}*/
		...
		$queryPlanes = "select PLAN_DATE as Fecha_Plan,
			...
			(select top 1 info from visitinst where visitinst.inst_snr=vp.inst_snr and visit_date in (select max(visit_date) from visitinst))  info_Ult_Vis, 
			c.NAME as ciclo
			from visinstplan vp
		...
		$queryCiclos = "select substring(ciclos.NAME,1,4) anio, 
				...
				where vp.PLAN_DATE between ciclos.start_date 
				and ciclos.finish_date 
				and vp.rec_stat=0 ";
				if($tipoUsuario == 4){
		...
		echo "<script>
				...
				$('#lblField_01').text('".$reg['field_01']."');
				$('#lblField_01_snr').text('".$reg['field_01_snr']."');
				";
				
		/*if($reg['idTipo'] == 1){
			echo "$('#divPerfilHospitales').show();";
		}else if($reg['idTipo'] == 2){
			echo "$('#divPerfilFarmacias').show();";
		}*/
		...
		/*representantes asinados a la institución*/
		
		$qRepres = "select top 1 u.LNAME + ' ' + MOTHERS_LNAME + ' ' + fname as nombre,
			u.TEL1, u.MOBILE,
			RIGHT('00' + cast(day(VISIT_DATE) as nvarchar), 2) + '/' + 
			RIGHT('00' + cast(month(VISIT_DATE) as nvarchar), 2) + '/' + 
			RIGHT('0000' + cast(year(VISIT_DATE) as varchar),4) as fecha
			from USER_TERRIT ut
			inner join users u on ut.USER_SNR = u.USER_SNR
			left outer join VISITINST v on v.INST_SNR = ut.INST_SNR
			where ut.INST_SNR = '".$id."'
			order by v.VISIT_DATE desc";
			
		$rsRepres = sqlsrv_query($conn, $qRepres);
		
		$arrRepre = sqlsrv_fetch_array($rsRepres);
		
		echo "$('#tblRepresentantesInst tbody').empty();
			$('#tblRepresentantesInst tbody').append('<tr><td style=\"width:50%;\">".$arrRepre['nombre']."</td><td style=\"width:20%;\">".$arrRepre['TEL1']."</td><td style=\"width:20%;\">".$arrRepre['MOBILE']."</td><td style=\"width:20%;\">".$arrRepre['fecha']."</td></tr>');
		";
		
		/*fin representantes asinados a la institución*/
