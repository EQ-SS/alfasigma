<?php
	...
	if($tipo == 'p'){
		$queryCampos = "SELECT CFT.SORT_NUM AS ORDEN, 
			...
			CFT.LONG_TEXT, CFT.SHORT_TEXT 
			FROM CONFIG_FIELD_TRANSLATION CFT 
			inner join CONFIG_FIELD cf on cf.CONF_FIELD_SNR = cft.CONF_FIELD_SNR
			INNER JOIN CONFIG_TABLE CT ON CF.TABLE_NR = CT.TABLE_NR 
			WHERE Ct.TABLE_NR IN (456, 13, 8, 7, 481) 
			AND CFT.LANG_NR = 8 AND CFT.SORT_NUM > 0 
			UNION 
			SELECT CFT.SORT_NUM AS ORDEN, 
			CASE WHEN CF.NAME = 'NAME' THEN CFT.LONG_TEXT ELSE CF.NAME END AS NAME,
			CFT.LONG_TEXT, 
			CASE WHEN CF.NAME = 'I_STREET1' THEN 18 WHEN CF.NAME = 'I_NUM_EXT' THEN 19 WHEN CF.NAME = 'I_NAME' THEN 17 ELSE SHORT_TEXT END AS SHORT_TEXT 
			FROM CONFIG_FIELD_TRANSLATION CFT 
			INNER JOIN CONFIG_FIELD CF ON CFT.CONF_FIELD_SNR = Cf.CONF_FIELD_SNR
			INNER JOIN CONFIG_TABLE CT ON CT.TABLE_NR=CF.TABLE_NR 
			WHERE CF.TABLE_NR IN (492) AND CFT.LANG_NR = 8 AND CFT.SORT_NUM > 0 AND CF.NAME IN ('I_STREET1','I_NUM_EXT', 'I_NAME') 
			ORDER BY ORDEN";
		...
		$queryPersApp = "select 
			...
			pat.name as P_PATPERWEEK_SNR,
			hon.name as P_FEE_TYPE_SNR,
			p.P_BIRTHDATE,
			...
			left outer join CODELIST pat on pat.clist_snr = p.P_PATPERWEEK_SNR 
			left outer join CODELIST hon on hon.clist_snr = p.P_FEE_TYPE_SNR
		...
		/*$desConection = '';
		if($regPersApproval['P_CONECTION_CRM_SNR'] != '' && $regPersApproval['P_CONECTION_CRM_SNR'] != '00000000-0000-0000-0000-000000000000'){
			if(substr($regPersApproval['P_CONECTION_CRM_SNR'], -1) == ';'){
				$conection = substr($regPersApproval['P_CONECTION_CRM_SNR'], 0, -1);
			}else{
				$conection = $regPersApproval['P_CONECTION_CRM_SNR'];
			}
			...
			while($conec = sqlsrv_fetch_array($rsConnection)){
				$desConection .= $conec['name'].",";
			}
			$regPersApproval['P_CONECTION_CRM_SNR'] = substr($desConection, 0, -1);
		}*/
		...
		//echo "mmmmmmmmm: ".$approvalStatus."mmmmmmmm";
		if($approvalStatus == 'C'){
			...
			$queryPers = "select 
				...
				left outer join DISTRICT dis on dis.DISTR_SNR = CITY.DISTR_SNR
				left outer join STATE edo on edo.STATE_SNR = city.STATE_SNR
				left outer join CODELIST pat on pat.clist_snr = p.PATPERWEEK_SNR 
	...
	//echo "---".$queryPers."<br><br>";
			...
			/*$desConection = '';
			if($regPers['P_CONECTION_CRM_SNR'] != '' && $regPers['P_CONECTION_CRM_SNR'] != '00000000-0000-0000-0000-000000000000'){
				if(substr($regPers['P_CONECTION_CRM_SNR'], -1) == ';'){
					$conection = substr($regPers['P_CONECTION_CRM_SNR'], 0, -1);
				}else{
					$conection = $regPers['P_CONECTION_CRM_SNR'];
				}
				$conection = str_replace(";", "','", $conection);
				$rsConnection = sqlsrv_query($conn, "select name from codelist where clist_snr in ('".$conection."')");
				while($conec = sqlsrv_fetch_array($rsConnection)){
				...
				$regPers['P_CONECTION_CRM_SNR'] = substr($desConection, 0, -1);
			}else{
				$regPers['P_CONECTION_CRM_SNR'] = '';
			}*/
			...
			if(sqlsrv_num_rows($rsAppChange) == 0){//entro por android
				...
				$qPersApproval = "select 
					...
					left outer join CODELIST pat on pat.clist_snr = pa.P_PATPERWEEK_SNR 
					left outer join CODELIST hon on hon.clist_snr = pa.P_FEE_TYPE_SNR
					...
					if(array_key_exists($arrayNombres[$m], $arrPersApproval)){
						if($arrPerson[$arrayNombres[$m]] != $arrPersApproval[$arrayNombres[$m]]){
							if( is_object($arrPerson[$arrayNombres[$m]]) || is_object($arrPersApproval[$arrayNombres[$m]])){
								...
								echo "$('#tblAprobacion').append('<tr><td><b>".$arrayValores[$m]."</b></td><td>".utf8_encode($arrPerson[$arrayNombres[$m]])."</td><td><font color=\"#3F51B5\">".utf8_encode($arrPersApproval[$arrayNombres[$m]])."</font></td></tr>');";
							}
						}
					}
				}
				...
				while($regAppChange = sqlsrv_fetch_array($rsAppChange)){
					...
					if($indice > -1){
						if(is_object($regPersApproval[$arrayNombres[$indice]])){
							...
							//echo "regPersApproval: ".$regPersApproval[$arrayNombres[$indice]]."<br><br>";
							//echo "regPers: ".$regPers[$arrayNombres[$indice]]."<br><br>";
		...
		$queryCampos = "select UPPER(case when rc.name = 'name' then rt.LONG_TEXT else rc.name end) as name, rt.LONG_TEXT, rt.SHORT_TEXT 
			...
			where rc.TABLE_NR = rb.TABLE_NR
			and rb.TABLE_NR in (492,13,8,7)
			and rt.LANG_NR = 8
			/*and rc.TABLE_NR = rb.TABLE_NR
			and rt.COLUMN_NR = rc.COLUMN_NR*/
			and rt.conf_field_snr = rc.conf_field_snr
			and rt.SORT_NUM > 0
