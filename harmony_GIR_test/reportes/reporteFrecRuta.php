<?php
	...
	$qMedicos = "DECLARE @CICLO as VARCHAR(36)
		...
		,MedsVis_Frec2_Ciclo1 as (Select VP.pers_snr, VP.user_snr from visitpers VP, person P
		where VP.pers_snr = P.pers_snr
		and VP.rec_stat = 0
		and VP.visit_date between @DIAS_IN and @DIAS_FIN
		and P.status_snr = @STATUS 
		and P.frecvis_snr = @FREC2
		...
		group by VP.pers_snr, VP.user_snr
		having count(VP.pers_snr)>1 )
		
		,MedsVis_Frec2_Ciclo2 as (Select VP.pers_snr, VP.user_snr from visitpers VP, person P
		where VP.pers_snr = P.pers_snr
		and VP.rec_stat = 0
		and VP.visit_date between (select start_date from Ciclo2) and (select finish_date from Ciclo2)
		and P.status_snr = @STATUS 
		and P.frecvis_snr = @FREC2
		and VP.visit_code_snr in ('146AA26A-502A-407A-A486-18470C9E7F23','2B3A7099-AC7D-47A3-A274-F0B029791801') /*CONTACTO VIRTUAL - PRESENCIAL*/
		group by VP.pers_snr, VP.user_snr
		...
		,MedsVis_Frec2_Ciclo3 as (Select VP.pers_snr, VP.user_snr from visitpers VP, person P
		where VP.pers_snr = P.pers_snr
		and VP.rec_stat = 0
		and VP.visit_date between (select start_date from Ciclo3) and (select finish_date from Ciclo3)
		and P.status_snr = @STATUS 
		and P.frecvis_snr = @FREC2
		and VP.visit_code_snr in ('146AA26A-502A-407A-A486-18470C9E7F23','2B3A7099-AC7D-47A3-A274-F0B029791801') /*CONTACTO VIRTUAL - PRESENCIAL*/
		group by VP.pers_snr, VP.user_snr
		...
		where MedsVis.user_snr = MR.user_snr
		) as Vis_Frec2_Ciclo1, 
	...
	if($tipo == 1 || $tipo == 2){//excel
		...
		header("Content-Disposition: filename=reporteFrecuenciaRuta.xls");
		header("Pragma: no-cache");
		header("Expires: 0");
	}else if($tipo == 3){
		ob_start();
		require('../pdf/fpdf.php');
		$pdf=new FPDF('L', 'mm', array(1300,900));
	
		$pdf->AddPage();
		$pdf->SetFont('Arial','B',10);
		$pdf->setDisplayMode(100, 'continuous');
		$pdf->Cell(40,5,utf8_decode('Reporte Frecuencia por Ruta'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
			...
			$tabla .= '<table border="0">
				<tr>
					<td>
						<table>
							<tr>
								<td colspan="10" class="nombreReporte">Reporte Frecuencia por Ruta</td>
							</tr>
							<tr>
