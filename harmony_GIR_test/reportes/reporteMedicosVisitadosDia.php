<?php
	...
	$qMedicos = "DECLARE @fecha_inicio Date, 
		...
		/*
		set @fecha_inicio = '2021-08-02'
		set @fecha_fin = '2021-08-18'
		*/
		
		set @cad0='00000000-0000-0000-0000-000000000000'
		set @no_vis='C2B3B4FB-A2A3-4379-88EB-AFC3FCC825B3' /*ESTA INACTIVO*/
		set @OActs='5279D7F1-5EAB-42ED-9FF9-3F7600361CA1'
		set @VAcom='7A50E6CC-A1A5-4495-AB9B-BA22AB92CB4F'
		...
		and datepart(DW,DATEADD(DAY, nbr - 1, @fecha_inicio)) not in (1)
		and DATEADD(DAY, nbr - 1, @fecha_inicio) not in (select c_date from CYCLE_DETAILS where c_date between @fecha_inicio and @fecha_fin and rec_stat=0)
		)
		...
		select DR.user_snr, DR.DATE, DR.dayreport_snr, DC.day_code_snr, DC.value, cl.name,
		 (case when cl.name='VIAJE/TRASLADOS' then 1 
		when cl.name='VACACIONES' then 2 
		when cl.name='INCAPACIDAD' then 3
		when cl.name='CAPACITACION' then 4 
		when cl.name='JUNTAS' then 5
		when cl.name='TRABAJOS ESPECIALES' then 6
		/*when cl.name='HOME OFFICE' then 8
		when cl.name='SEMAFORO ROJO' then 9
		when cl.name='TRABAJO ADMINISTRATIVO' then 10
		when cl.name='VISITA CONJUNTA' then 11 */
		else 7 end) COLOR
		...
		(select isnull(sum(value),0) from DAY_REPORT DR, DAY_REPORT_CODE DC where DR.DATE between @fecha_inicio and @fecha_fin and datepart(DW,DR.DATE) not in (1) and DR.dayreport_snr = DC.dayreport_snr and DR.rec_stat=0 and DC.rec_stat=0 and DR.user_snr = U.user_snr)
		 as OAct_T
		...
		and DM.User_type = 5
		/*and U.user_snr <> 'C5B5BF12-AE52-47DC-ABED-B7EDB53D9120'*/ /*1020199 test*/
		and DM.user_nr not in ('100','200','300','001','002')
		and (U.user_snr in (select kloc_snr from kloc_reg where rec_stat=0 and kloc_snr in ('".$ids."') ))
		
		and exists (select * from Kommloc KL1 where KL1.rec_stat = 0 and kl1.Activated = 1 and kl1.kloc_snr = klr.kloc_snr)
		
		Order by DM.lname,DM.Fname,U.USER_TYPE,U.Lname,U.Fname,klr.REG_SNR ";
				...
				<tr>
					<td>
						<table>
							...
							<tr>
								<td colspan="2" class="clienteReporte">Torrent</td>';
								if($tipo != 2){								
									$tabla .= '<td>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</td>
									<td style="background-color: #ABEBC6; border: 1px solid #000;" width="20px"> </td><td>Capacitacion &nbsp;</td>
									<td style="background-color: #FEF5E7; border: 1px solid #000;" width="20px"> </td><td>Juntas &nbsp;</td>
									<td style="background-color: #D7BDE2; border: 1px solid #000;" width="20px"> </td><td>Incapacidad &nbsp;</td>
									<td style="background-color: #F5B7B1; border: 1px solid #000;" width="20px"> </td><td>Trabajos Especiales &nbsp;</td>
									<td style="background-color: #D6EAF8; border: 1px solid #000;" width="20px"> </td><td>Vacaciones &nbsp;</td>
									<td style="background-color: #FFFF00; border: 1px solid #000;" width="20px"> </td><td>Viaje/Translados &nbsp;</td>
									</tr>';
								}
							$tabla .= '<tr>
	...
	while($reg = sqlsrv_fetch_array($rsMedicos)){	
		...
		if ($totalVisitas > 0 && $totalCuotaT > 0){
		...
		for($l = 1; $l <= $numDias; $l++){ 
			...
			$totalVisDia[$con1d] += $sumtotalVisDia[$con1d];
			if ($totalVisDia[$con1d] > 0 && $totalCuota > 0){
			...
			if($reg['RM'] == $reg['SR']){
				...
				for($l = 1; $l <= $numDias; $l++){ 
					switch($reg[$color]){
						...
						case 1: $estilocolor = 'background-color: #FFFF00;"'; break; //Viaje/Translados
						...
						case 5: $estilocolor = 'background-color: #FEF5E7;"'; break; //Juntas
						case 6: $estilocolor = 'background-color: #F5B7B1;"'; break; //Trabajos Especiales
			...
			if($reg['RM'] == $reg['SR']){
				...
				for($l = 1; $l <= $numDias; $l++){
					switch($reg[$color]){
						...
						case 1: $pdf->SetFillColor(255,255,0); break; //Viaje/Translados
						...
						case 5: $pdf->SetFillColor(254,245,231); break; //Juntas
						case 6: $pdf->SetFillColor(245,183,177); break; //Trabajos Especiales
