...
<?php
	...
	$nombreListado="Listado_TFT";
	
	$qMedicos = "with qry as (
	Select distinct 
	cl.name as Linea, 
	Ugte.user_nr as Gte,
	U.user_nr as Ruta,
	upper(U.lname)+' '+upper(U.fname) as Representante,
	cast(cast(DR.date as DATE) as varchar(10)) as Fecha,
	act.name as Actividad,
	(select sum(value) from day_report_code where DC.dayreport_snr = day_report_code.dayreport_snr and rec_stat=0) as Total_Horas,
	((select sum(value) from day_report_code where DC.dayreport_snr = day_report_code.dayreport_snr and rec_stat=0) / 8 * 100) as Porc_Dia,
	(case when DC.value=0 then (select sum(value) from day_report_code where DC.dayreport_snr = day_report_code.dayreport_snr and rec_stat=0) else DC.value end) as Hrs_Actividad,
	((case when DC.value=0 then (select sum(value) from day_report_code where DC.dayreport_snr = day_report_code.dayreport_snr and rec_stat=0) else DC.value end) / 8 * 100) as Porc_Act,
	(select UC.user_nr+' - '+UC.lname+' '+UC.fname from users UC where UC.user_snr = DR.creator_user_snr) as Creado_por,
	DR.info as Comentarios,
	cast(cast(DR.creation_timestamp as DATE) as varchar(10)) as Fecha_creacion
	
	from day_report DR
	inner join day_report_code DC on DC.dayreport_snr = DR.dayreport_snr
	inner join users U on U.user_snr = DR.user_snr 
	...
	inner join codelist act on act.clist_snr = DC.day_code_snr and act.rec_stat=0
	...
	/*and DR.date between '2023-01-01' and '2023-05-30' */ 
	)
	
	select Linea,Gte,Ruta,Representante,Fecha,Actividad,Total_Horas,Porc_Dia,Hrs_Actividad,Porc_Act,Creado_por,Comentarios,Fecha_creacion 
	from qry
	order by Ruta,Fecha,Actividad ";
