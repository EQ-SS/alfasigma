<?php
	...
	if(! $conn){
		...
		echo "$('#lstEstrategia".$combo."').empty();";
		$rsEstrategia = llenaCombo($conn, 255, 1);
		echo "$('#lstEstrategia".$combo."').append('<option value=\"00000000-0000-0000-0000-000000000000\">Seleccione</option>');";
		while($estrategia = sqlsrv_fetch_array($rsEstrategia)){
			echo "$('#lstEstrategia".$combo."').append('<option value=\"".$estrategia['id']."\">".$estrategia['nombre']."</option>');";
		}
		echo "</script>";
		//echo $queryProductos;
	}
