<?php
	...
	$queryGrafica6 = "with chart2 as (select
        p.pers_snr,
        isnull(cat_audit.name,'NC') as categ,
		...
		/*$total = ($visitados / $registrados)*100;
		print_r($registrados);*/
		...
		for($i=0;$i<count($categorias);$i++){
			...
			if($categorias[$i] == 'NC'){
				$datos2 .= '{"label": "NC", "value": "'.$por.'"},';
			}else{
				$datos2 .= '{"label": "Quintil '.$categorias[$i].'", "value": "'.$por.'"},';
			}
		}
		...
		$datos2 .= '{"label": "TOTAL", "value": "'.$porTotal.'"}';
