<?php
	...
	function actualizaTablas($conn, $idPers, $idUsuario, $ruta, $tipoUsuario, $repre){
		...
		while($visita = sqlsrv_fetch_array($rsVisitas)){
			...
			//echo "$('#tblVisitas tbody').append('<tr onClick=\"muestraVisita(\'".$visita['vispers_snr']."\',\'".$visita['idPlan']."\');\"><td style=\"width:15%;\">".$fecha_vis."</td><td style=\"width:10%;\">".$visita['time']."</td><td style=\"width:35%;\">".$visita['Rep']."</td><td style=\"width:30%;\">".utf8_encode($visita['Tipo_Vis'])."</td><td style=\"width:10%;\">".substr($visita['ciclo'],0,5).substr($visita['ciclo'],8,2)."</td></tr>');";
			...
			//echo "$('#tblVisitas tbody').append('<tr onClick=\"muestraVisita(\'".$visita['vispers_snr']."\',\'".$visita['idPlan']."\');\"><td style=\"width:11%;\">".substr($visita['ciclo'],0,5).substr($visita['ciclo'],8,2)."</td><td style=\"width:8%;\">".$visita['user_nr']."</td><td style=\"width:13%;\">".$fecha_vis."</td><td style=\"width:9%;\">".$visita['time']."</td><td style=\"width:19%;\">".utf8_encode($visita['Tipo_Vis'])."</td><td style=\"width:20%;\">".$visita['informacion_vis']."</td><td style=\"width:20%;\">".$visita['obj_vis']."</td></tr>');";
		}
	...
	if(! $conn){
		...
		$comentariosVisita = strtoupper($_POST['comentariosVisita']);
		$infoSiguienteVisita = strtoupper($_POST['infoSiguienteVisita']);
		...
		$repre = $_POST['repre'];
		$visitaAcompa = ($_POST['visitaAcompa'] == '') ? '00000000-0000-0000-0000-000000000000' : $_POST['visitaAcompa'];
		...
		//echo "prod: ".$_POST['productosSeleccionados'];
		...
		if($idVisita == ''){//nueva visita
			...
			$query = "insert into visitpers (
				...
				'".$idPers."',
				'".$codigoVisita."',
				'".utf8_decode(strtoupper($comentariosVisita))."',
				'".$pwork."',
				0,
				'".$idUsuario."',
				'".$fecha."',
				...
				0,
				'".$idPlan."',
				'".utf8_decode(strtoupper($infoSiguienteVisita))."',
				'".$motivoNoVisita."'
			)"; 
			...
			$query = "update visitpers set 
				...
				INFO = '".utf8_decode(strtoupper($comentariosVisita))."',
				SYNC = 0,
				INFO_NEXTVISIT = '".utf8_decode(strtoupper($infoSiguienteVisita))."',
				NOVIS_SNR = '".$motivoNoVisita."' 
		...
		if(! empty($_POST['productosSeleccionados'])){
			for($i=0;$i<count($arrProductosSeleccionados);$i++){
				$pos = $i+1;
				if($arrProductosSeleccionados[$i] != 'null'){
