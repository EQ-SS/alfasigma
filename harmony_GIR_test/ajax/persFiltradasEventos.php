	...
	if(! $conn){
		...
		if($palabra == ''){
			...
			while($persona = sqlsrv_fetch_array($rsPersonas)){
				...
				$nombre = str_ireplace($buscar,$reemplazar,utf8_encode(strtoupper(str_replace("'", "", $persona['nombre']))));
				$especialidad = str_ireplace($buscar,$reemplazar,utf8_encode(str_replace("'", "", $persona['especialidad'])));
				$institucion = str_ireplace($buscar,$reemplazar,utf8_encode(str_replace("'", "", $persona['institucion'])));
				if($tipoUsuario == 4){
			...
			while($persona = sqlsrv_fetch_array($rsPersonas)){
				$ruta = explode(" ",$persona['ruta'])[0];
				if($tipoUsuario == 4){
					echo "$('#tblBuscarPersonasEventos tbody').append('<tr><td style=\"width:34%;\">".utf8_encode(strtoupper(str_replace("'", "", $persona['nombre'])))."</td><td style=\"width:34%;\">".utf8_encode($persona['especialidad'])."</td><td style=\"width:32%;\">".utf8_encode($persona['institucion'])."</td></tr>');";	
				
				}else{
					echo "$('#tblBuscarPersonasEventos tbody').append('<tr><td style=\"width:10%;\">".$ruta."</td><td style=\"width:30%;\">".utf8_encode(strtoupper(str_replace("'", "", $persona['nombre'])))."</td><td style=\"width:30%;\">".utf8_encode($persona['especialidad'])."</td><td style=\"width:30%;\">".utf8_encode($persona['institucion'])."</td></tr>');";	
				}
			}
