	...
	if(! $conn){
		...
		if(isset($_POST['idVisita']) && $_POST['idVisita'] != '' && $_POST['idVisita'] != '00000000-0000-0000-0000-000000000000'){
			...
			$query = "select 
				...
				time,
				u.user_nr + ' - ' + u.LNAME + ' ' + u.MOTHERS_LNAME + ' ' + u.FNAME  as Rep,
		...
		if($tipoUsuario == 4){
			...
			if($tipoUsuario == 2){
				...
				echo "$('#sltRepreVisita').append('<option value=\"".$visita['user_snr']."\" >".$visita['repre']."</option>');";
				echo "$('#sltRepreVisita').append('<option value=\"".$regSuper['USER_SNR']."\" selected> ".$regSuper['USER_NR']." - ".$regSuper['LNAME']." ".$regSuper['FNAME']."</option>');";
			}
		}
		...
		if($idVisita == ''){
			...
			/*echo "select mplan.promotion_enabled 
				from MARKETING_PLAN mplan
				inner join cycles cycle on cycle.CYCLE_SNR=mplan.CYCLE_SNR
				inner join marketing_plan_specialty mplan_spec on mplan_spec.mplan_snr=mplan.mplan_snr and mplan_spec.rec_stat=0
				inner join CODELIST especialidad on especialidad.CLIST_SNR = mplan_spec.SPEC_SNR 
			...
			if($promotion_enabled == 1){
				$qProductos = "select
					...
					cast(mplan_spec_prod.percentage * 100 as int) as percentage
					from marketing_plan mplan
					inner join marketing_plan_specialty mplan_spec on mplan_spec.mplan_snr=mplan.mplan_snr and mplan_spec.rec_stat=0
					...
					and mpu.USER_SNR = '".$idUsuario."' 
					order by specialty.name,mplan_spec_prod.percentage desc ";
			...
			/******** concepto pago medico   agregar visita     inicio ***********/
			...
			$queryComision = "
                select 
                    ...
                    c.month,
                    c.year, 
                    c.amount_comission as comision, 
                    c.concept,
					c.vispers_snr

				from USER_PERSON_COMISSION c where c.PERS_SNR = '".$idPersona."'
                and c.USER_SNR = '".$idUser."'
				and c.VISPERS_SNR = '00000000-0000-0000-0000-000000000000'
				and c.REC_STAT = 0 
                ORDER BY month,  year
			";
			...
			$rsQueryComision = sqlsrv_query($conn, $queryComision, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
			$total=0;
			$lineas = sqlsrv_num_rows($rsQueryComision);
			//echo "$('#tblPagoMed tbody').append('".$id."');";
			//print_r ("$('#tblPagoMed tbody').append('".$idUser." --- ".$idPersona."');");
			if ($lineas > 0){
				while($rsComision = sqlsrv_fetch_array($rsQueryComision)){
					$mes = mesletra($rsComision['month']);
					echo "$('#tblComision tbody').append('<tr><td>".$rsComision['concept']."</td><td>".$mes."</td><td>".$rsComision['year']."</td><td>$ ".$rsComision['comision']."</td></tr>');";
					$total=$total + $rsComision['comision'];
				}
			}
			echo "$('#tblComision tbody').append('<tr><td>  </td><td>  </td><td>Total a pagar</td><td id=\"total\"> $ ".$total."</td></tr>');";
			...
			if(sqlsrv_num_rows($rsProdcutos)>0){
				...
				$renglon .= '</select><div class="select_arrow"></div></div></td><td><input class="form-control" id="txtProdPos1" type="text" value="" size="3" /></td></tr>';
			...
			/******** concepto pago medico   con id     inicio ***********/
			...
			$queryComision = "
				...
				from USER_PERSON_COMISSION c where  c.VISPERS_SNR = '".$idVisita."'
				and c.REC_STAT = 0 
                ORDER BY month,  year
			";
			...
			$total=0;
			$lineas = sqlsrv_num_rows($rsQueryComision);
			//echo "$('#tblMuestras tbody').append('".$idvis."');";
			//print_r ("$('#tblPagoMed tbody').append('".$idUser." --- ".$idPersona."');");
			if ($lineas > 0){
				while($rsComision = sqlsrv_fetch_array($rsQueryComision)){
					$mes = mesletra($rsComision['month']);
			...
			echo "$('#tblComision tbody').append('<tr><td>  </td><td>  </td><td>Total a pagar</td><td id=\"totalComision\"> $ ".$total."</td></tr>');";
		...
		if($firma != ''){
			echo "
				...
				$('#imgFirmaComision').attr('src', 'data:image/png;base64,".str_ireplace($buscar,$reemplazar1,str_replace(" ","+",trim($firma)))."');
				$('#f64').val('".str_ireplace($buscar,$reemplazar,trim($firma))."');
				";
		}else{
			echo "
			$('#imgFirmaComision').hide();
			$('#tblFirmaComision').show();
			";
		}
