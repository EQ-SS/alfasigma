<?php
	...
	if($tipo == 'p'){
		...
		$queryPersApp = "select 
			...
			left outer join CODELIST pacientes on pacientes.CLIST_SNR = p.P_PATPERWEEK_SNR
	...
	//echo "---".$queryPers."<br><br>";
			...
			if(sqlsrv_num_rows($rsAppChange) == 0){//entro por android
				...
				$qPersApproval = "select 
					...
					left outer join CODELIST frec on frec.CLIST_SNR = pa.p_FRECVIS_SNR
					where at.APPROVAL_STATUS_SNR = '".$idApproval_status."'";
			...
			if(sqlsrv_num_rows($rsAppChange) == 0){//entro por android
				...
				for($i=0;$i<count($arrayNombres);$i++){
					if($regInstApproval[$arrayNombres[$i]] != $regInst[$arrayNombres[$i]]){
						...
						//echo "<script>$('#tblAprobacion').append('<tr><td><b>".$arrayValores[$i]."</b></td><td>".$regInstApproval[$arrayNombres[$i]]."</td><td>".$regInst[$arrayNombres[$i]]."</td></tr>');</script>";
					}
				}
