<?php
	...
	//tab Representante Inst
	$queryRepreInst="SELECT U.USER_NR+' - '+U.LNAME+' '+U.MOTHERS_LNAME+' '+U.FNAME AS REPRE 
	,I.TEL1
	,I.TEL2
	,(SELECT TOP 1 CONVERT(VARCHAR(10),VISIT_DATE,120)  as fecha
	FROM VISITINST WHERE INST_SNR=I.INST_SNR AND REC_STAT=0) AS FECHA_VISITIA
	FROM USER_TERRIT UT 
	INNER JOIN USERS U ON U.USER_SNR=UT.USER_SNR
	INNER JOIN INST I ON I.INST_SNR=UT.INST_SNR
	WHERE UT.UTER_SNR <>'00000000-0000-0000-0000-000000000000' AND UT.REC_STAT=0
	AND UT.INST_SNR='".$id."'
	ORDER BY U.USER_NR,U.LNAME,U.MOTHERS_LNAME,U.FNAME";
	$rsRepreInst = sqlsrv_query($conn, $queryRepreInst);

	echo "<script>$('.tblPlanesVisitas tbody').empty();</script>";

	while($repre = sqlsrv_fetch_array($rsRepreInst)){
		echo "<script>$('.tblPlanesVisitas tbody').append('<tr ><td style=\"width:50%;\">".$repre['REPRE']."</td><td style=\"width:25%;\">".$repre['TEL1']."</td><td style=\"width:25%;\">".$repre['TEL2']."</td><td style=\"width:30%;\">".$repre['FECHA_VISITIA']."</td></tr>');</script>";
	}
	//END tab Representante Inst
?>