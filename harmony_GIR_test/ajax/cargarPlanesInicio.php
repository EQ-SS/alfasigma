<?php
	...
	if(! $conn){
		...
		//echo "ids: ".$ids."<br>";
		$queryPlanes = "select top 10 'p' as tipo, vpp.VISPERSPLAN_SNR as idPlan, 
			p.fname + ' / ' + p.LNAME + ' ' + p.mothers_lname as nombre, cl.NAME as especialidad, i.street1, t.ZIP, 
			...
			and vpp.rec_stat = 0 ";
			$queryPlanes .= " union
			select top 10 'i' as tipo, vpp.VISINSTPLAN_SNR as idPlan, i.name as nombre, '' as especialidad,  i.street1, t.ZIP,
			t.name as colonia, brick.NAME as brick, vpp.PLAN_DATE, 
			vpp.TIME AS VRIJEME, u.user_nr as ruta, 
		...
		while($plan = sqlsrv_fetch_array($rsPlanes)){
			...
			if($tipoUsuario == 4){
				...
				if($plan['tipo'] == 'p'){
					echo "$('#tblPlanesInicio tbody').append('<tr onclick=\"muestraPlan(\'".$plan['idPlan']."\')\"><td width=\"16%\" align=\"center\">".explode(" ",$plan["ruta"])[0]."</td><td width=\"50%\"><b>".$color."<br>".utf8_encode($plan["especialidad"])."</b><br>".utf8_encode($plan["street1"]).", ".$plan["ZIP"].', '.utf8_encode($plan["colonia"])."<br>".$plan["brick"]."</td><td width=\"20%\" align=\"center\">".$fecha_plan."</font></td><td width=\"20%\" align=\"center\">".$plan["VRIJEME"]."</td></tr>');";
				}else{
					echo "$('#tblPlanesInicio tbody').append('<tr onclick=\"muestraPlanInst(\'".$plan['idPlan']."\')\"><td width=\"16%\" align=\"center\">".explode(" ",$plan["ruta"])[0]."</td><td width=\"50%\"><b>".$color."<br>".utf8_encode($plan["especialidad"])."</b><br>".utf8_encode($plan["street1"]).", ".$plan["ZIP"].', '.utf8_encode($plan["colonia"])."<br>".$plan["brick"]."</td><td width=\"20%\" align=\"center\">".$fecha_plan."</font></td><td width=\"20%\" align=\"center\">".$plan["VRIJEME"]."</td></tr>');";
				}
			}
		}
