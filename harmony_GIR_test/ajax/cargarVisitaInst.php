<?php
	...
	if($idVisita == ''){
		...
		///echo "<script>alert('".$tipoInst."');</script>";
		if($tipoInst == 1){
			$visitCode = "DC017477-79A2-4935-B9C0-EC781FD9EC79";
		}else if($tipoInst == 2){
			$visitCode = "187C05A4-173E-4ED0-B9C9-4D41193E5E98";
		}else if($tipoInst == 3){
			$visitCode = "797044D0-33AD-4071-BEFE-2597AD872825";
		}else if($tipoInst == 4){
			$visitCode = "5DFDA7A0-19DC-408C-846C-16554E779AE1";
		}
	...
	echo "<script>
		...
		$('#txtComentariosVisitaInst').val('".str_ireplace($buscar,$reemplazar,utf8_encode($infoVisita))."');
		$('#txtInfoSiguienteVisitaInst').val('".str_ireplace($buscar,$reemplazar,utf8_encode($infoSigVisita))."');
	...
	if($tipoInst == 2){//pharmacia 
		$rsCodigo = llenaCombo($conn, 34, 9);
	}else if($tipoInst == 1){//hospital
		$rsCodigo = llenaCombo($conn, 34, 9);
	}else if($tipoInst == 3){//Consultorios
		$rsCodigo = llenaCombo($conn, 34, 9);
	}else if($tipoInst == 4){//Clinicas
		$rsCodigo = llenaCombo($conn, 34, 9);
	}
	...
	if($visitCode == '0E0249F6-8584-4ACA-BB91-23F4EB726EE9'){//no visita
	...
	if($idVisita == ''){
		...
		$renglon .= '<td><div class="select"><select id="lstEstrategiaInst1" class="form-control">';
		...
		$renglon .= '</select><div class="select_arrow"></div></div></td>';
		$renglon .= '<td><input class="form-control" type="text" id="txtProdPosInst1" value="" size="3" readonly /></td></tr>';
...
*/			
		...
		if(sqlsrv_num_rows($rsProdcutos)>0){
			$cuentaProducto = 1;
			while($producto = sqlsrv_fetch_array($rsProdcutos)){
				echo "$('#tblProductosVisitasInst tbody').append('<tr class=\"align-center\"><td>".$cuentaProducto."</td><td><div class=\"select\"><select class=\"form-control\"><option>".$producto['producto']."</option></select></select><div class=\"select_arrow\"></td><td><div class=\"select\"><select  class=\"form-control\"><option>".$producto['estrategia']."</option></select><div class=\"select_arrow\"></div></div></td><td><input class=\"form-control\" type=\"text\" value=\"".$producto['porcentaje']."\" size=\"3\" readonly /></td></tr>');";
				$cuentaProducto++;
			}
			for($i=$cuentaProducto;$i<11;$i++){
				echo "$('#tblProductosVisitasInst tbody').append('<tr class=\"align-center\"><td>".$i."</td><td><div class=\"select\"><select class=\"form-control\"><option>Seleccione</option></select></select><div class=\"select_arrow\"></td><td><div class=\"select\"><select class=\"form-control\"><option>Seleccione</option></select></select><div class=\"select_arrow\"></td><td><input class=\"form-control\" type=\"text\" value=\"\" size=\"3\" readonly /></td></tr>');";
			}
		}else{
			$queryProductos = "select * from product where REC_STAT = 0 order by name";
			$rsProdcutos = sqlsrv_query($conn, $queryProductos);
			...
			$renglon .= '</select><div class="select_arrow"></div></div></td>';
			$renglon .= '<td><input class="form-control" id="txtProdPosInst" type="text" value="" size="3" readonly /></td></tr>';
			echo "$('#tblProductosVisitasInst tbody').append('".$renglon."');";
			for($i=2;$i<11;$i++){
				echo "$('#tblProductosVisitasInst tbody').append('<tr class=\"align-center\"><td>".$i."</td><td><select class=\"form-control\" id=\"lstProductoInst".$i."\" onchange=\"llenaSiguienteVisitasInst(".$i.");\"><option>Seleccione</option></select></td><td><select class=\"form-control\" id=\"lstEstrategiaInst".$i."\"><option>Seleccione</option></select></td><td><input class=\"form-control\" id=\"txtProdPosInst".$i."\" type=\"text\" value=\"\" size=\"3\" /></td></tr>');";
			}
		}
		
		$queryStock = "select p.name producto, 
			pf.name presentacion, 
			ipf.quantity existencia/*, 
			ipf.out_of_stock_snr agotado, 
			ipf.SUGGESTED pedido, ipf.chain_snr cadena, 
			ipf.current_promotion_snr promociones*/
		...
		while($stock = sqlsrv_fetch_array($rsStock)){
			$arrAgotado = sqlsrv_fetch_array(sqlsrv_query($conn, "select name from codelist where clist_snr = '".$stock['agotado']."'"));
		$arrCadena = sqlsrv_fetch_array(sqlsrv_query($conn, "select name from codelist where clist_snr = '".$stock['cadena']."'"));
		$arrPromociones = sqlsrv_fetch_array(sqlsrv_query($conn, "select name from codelist where clist_snr = '".$stock['promociones']."'"));
			$tablaStock = "<tr>";
			$tablaStock .= "<td width=\"47%\" >".$stock['producto']." ".$stock['presentacion']."</td>";
			$tablaStock .= "<td width=\"10%\" align=\"center\">".$stock['existencia']."</td>";
			$tablaStock .= "<td width=\"10%\" align=\"center\">".$arrAgotado['name']."</td>";
			$tablaStock .= "<td width=\"10%\" align=\"center\">".$stock['pedido']."</td>";
			$tablaStock .= "<td width=\"15%\" align=\"center\">".$arrCadena['name']."</td>";
			$tablaStock .= "<td width=\"15%\" align=\"center\">".$arrPromociones['name']."</td>";
			$tablaStock .= "</tr>";
			echo "$('#tblProductosStockConsulta tbody').append('".$tablaStock."');";
		}
	}
	...
	echo "$('#hdnTotalChecksInst').val('".$contadorChecks."');
		$('#hdnDescripcionChkVisitasInst').val('".utf8_encode($descripcionesCheck)."');
		$('#sltMultiSelectInstVisita').text('".utf8_encode($descripcionesCheck)."');";
