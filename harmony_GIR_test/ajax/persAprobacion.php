	...
	if(! $conn){
		...
		if($tipoUsuario == 5){
			$queryApprovalStatus = "update APPROVAL_STATUS set 
				APPROVED_DATE_L1 = getdate(),
				APPROVED_L1_USER_SNR = '$idUserApproved',
				APPROVED_STATUS_L1 = 2
				where APPROVAL_STATUS_SNR = '$idApprovalStatus' ";
				
			if(! sqlsrv_query($conn, $queryApprovalStatus)){
				echo "no autorizó gerente: ".$queryApprovalStatus."<br>";
			}
			...
			$nombreAsistente = $arrPersApproval['P_ASSISTANT_NAME'];
			$telAsistente = $arrPersApproval['P_ASSISTANT_TEL'];
			$mailAsistente = $arrPersApproval['P_ASSISTANT_EMAIL'];
			$frecuencia = ($arrPersApproval['P_FRECVIS_SNR'] == '') ? '00000000-0000-0000-0000-000000000000' : $arrPersApproval['P_FRECVIS_SNR'];
			...
			/*$nombreHospital = $arrPersApproval['P_HOSPITAL_NAME'];
			$preferenciaContacto = $arrPersApproval['P_PREFERRED_CONTACT_SNR'];
			$aceptaApoyo = $arrPersApproval['P_ACCEPT_SUPPORT_SNR'];
			$porqueAceptaApoyo = $arrPersApproval['P_ACCEPT_SUPPORT_INFO'];
			$botiquin = $arrPersApproval['P_AID_KIT_SNR'];
			$compraDirecta = $arrPersApproval['P_DIRECT_PURCHASE_SNR'];*/
			$liderOpinion = $arrPersApproval['P_KOL_SNR'];
			$speaker_snr = $arrPersApproval['P_SPEAKER_SNR'];
			/*
			$tipoConsulta = $arrPersApproval['P_CONSULTATION_TYPE_SNR'];*/

			$pasatiempo = $arrPersApproval['P_BASIC_LIST_SNR'];
			...
			$padecimientosMedicos = $arrPersApproval['P_AILMENT_SNR'];
			$estadoCivil = $arrPersApproval['P_MARITAL_STATUS_SNR'];
			$brightly_snr = $arrPersApproval['P_BRIGHTLY_SNR'];
