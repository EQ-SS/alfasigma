		...
		<div class="card m-b--15 card-add-new">
			<div class="header row padding-0">
				...
				<div class="col-lg-6 col-md-6 col-sm-10 col-xs-11 align-center m-t-10 m-b-10 display-inline">
					...
					<button id="btnVisitaEnEspera" type="button" class="btn bg-indigo waves-effect btn-indigo m-l-10">
						Visita en Espera
					</button>
				...
				<div id="tabsVisitas">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding-0 m-t--20">
						<ul class="nav nav-tabs m-l--35 m-r--35 tab-col-blue p-l-5" role="tablist"
							...
							<!--  INICION COMISION  -->
							<li role="presentation">
								<a href="#tabComision" data-toggle="tab" class="show-tooltip p-t-14">
									<i class="fas fa-money-bill-wave-alt"></i>
									<div class="divTooltip">Comision</div>
									<span class="hidden-txt-tabs">Comision</span>
								</a>
							</li>
							<!--  FIN COMISION  -->
						</ul>
...
?>
													...
													<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
														<div class="form-group margin-0">
															<label>Motivo de no visita</label>
															<select id="lstMotivoNoVisita" class="form-control">
																<option value="00000000-0000-0000-0000-000000000000">Seleccione</option>
<?php
																$rsCodigo = llenaCombo($conn, 33,10);
																while($codigo = sqlsrv_fetch_array($rsCodigo)){
																	echo '<option value="'.$codigo['id'].'" >'.utf8_encode($codigo['nombre']).'</option>';
																}
?>
...
<?php
																	...
																	while($acom = sqlsrv_fetch_array($rsAcom)){
																		...
																		echo '<input type="checkbox" id="'.$idChk.'" class="filled-in chk-col-red"  onclick="agregaDesVisAcompa(\''.utf8_encode($acom['nombre']).'\',\''.$idChk.'\');" value="'.$acom['id'].'"/>';
																		echo '<label for="'.$idChk.'">'.utf8_encode($acom['nombre']).'</label>';
																	}
?>
																</div>
															</div>
							...
							<div id="tabMuestras">
								...
								<div class="pull-center">
									...
									<div style="margin:50px; width: 500px; text-align: justify;" <?= (utf8_encode(textoFirma($conn, $_GET['idUser'])) == '') ? 'hidden' : '' ?>>
										<?= utf8_encode(textoFirma($conn, $_GET['idUser'])) ?>
									</div>
							...
							<!-- INICIO NUEVA TAB  COMISION-->
							<div id="tabComision">
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"
										style="max-height:190px; overflow:auto;">
										<table id="tblComision" class="table table-striped">
											<thead class="bg-cyan">
												<tr>
													<td class="negrita">Concepto Pago Médicos</td>
											...
											<tbody>
												<tr>
													<td class="negrita"><input class="form-control input-number" type="number" size="5" id="text" style="text-align:right"/></td>
													<td class="negrita"><input class="form-control input-number" type="number" size="5" id="text" style="text-align:right"/></td>
													<td class="negrita"><input class="form-control input-number" type="number" size="5" id="text" style="text-align:right"/></td>
													<td class="negrita"><input class="form-control input-number" type="number" size="5" id="text" style="text-align:right"/></td>
												</tr>
								...
								<div class="pull-center">
									...
									<div class="sigPad" id="tblFirmaComision" style="width:422px;">
										<ul class="sigNav pull-right">
											<li class="clearButton">
												<a href="#clear">
													<button id="btnLimpiarFirma" class="btn btn-default waves-effect btn-indigo2">
											...
											<li>
												<button id="btnGuardarFirma" onClick="guardarFirmaComision();" class="btn bg-light-blue waves-effect btn-light-blue">
													Guardar
												</button>
											</li>
										</ul>
										<div class="sig sigWrapper canvaPer">
											<div class="typed"></div>
											<canvas id="canvasFirmaVisitasComision" class="pad" width="400" height="250"></canvas>
											<input type="hidden" id="hdnFirmaComision" name="output-2" class="output">
										</div>
										<div style="margin:10px;" class="align-center">
											Firmar sin salir del recuadro
										</div>
									...
									<img id="imgFirmaComision"/>
									<input class="form-control" type="hidden" id="f64" />
								</div>
							</div>
							<!--  FIN NUEVA TAB COMISION -->
