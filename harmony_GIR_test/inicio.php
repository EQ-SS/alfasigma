<?php
	if(! isset($_GET['idUser']) || $_GET['idUser'] == ''){
		...
		/*$queryTipoUsuario = "select * 
			from users where 
			rec_stat = 0 
			and user_snr = '".$_GET['idUser']."'";
		...
		if(sqlsrv_num_rows($rsTipoUsuario) > 0){
			...
			}else if($tipoUsuario == 5)///es gerente
			{
				$queryUsuarios = "select USER_SNR from kloc_reg k, users u
						...
						/*and u.USER_TYPE = 4*/
						order by u.LNAME ";
			}else///repre o cualquier otro usuario
			{
				$ids = $_GET['idUser'];
			}
							...
							if($tipoUsuario == 2){
								echo '<div class="form-group display-flex margin-0 pull-right" style="width:50%;">
									...
									<select id="sltGte" class="form-control">
										...
										$queryGte = "select * from users where USER_TYPE = '5' order by user_nr, lname";
										$rsGte = sqlsrv_query($conn, $queryGte);
										while($regGte = sqlsrv_fetch_array($rsGte)){
											echo '<option value="'.$regGte["USER_SNR"].'">'.utf8_encode($regGte["USER_NR"].' - '.$regGte["LNAME"].' '.$regGte["FNAME"]).'</option>';				
										}
								echo '</select></div>';
							}
								...
								$queryRutas = "select * from users where user_snr in ('".$ids."') order by user_nr, lname";
								$rsRutas = sqlsrv_query($conn, $queryRutas);
								while($regRutas = sqlsrv_fetch_array($rsRutas)){
									echo '<option value="'.$regRutas["USER_SNR"].'">'.utf8_encode($regRutas["USER_NR"].' - '.$regRutas["LNAME"].' '.$regRutas["FNAME"]).'</option>';				
								}
								echo '</select></div>
								</div>';
						}
...
?>
												<label class="m-t-5 m-r-5">Ruta:</label>
													<select id="sltRutas" class="form-control add-m-b">
														<option value="">Todas las rutas</option>
<?php							
													$queryRutas = "select USER_SNR,USER_NR,LNAME,MOTHERS_LNAME,FNAME
														from users 
														where user_snr in ('".$ids."') 
														order by USER_NR";
													$rsRutas = sqlsrv_query($conn, $queryRutas);
													while($regRutas = sqlsrv_fetch_array($rsRutas)){
														echo '<option value="'.$regRutas["USER_SNR"].'">'.$regRutas["USER_NR"].' - '.utf8_encode($regRutas["LNAME"].' '.$regRutas["MOTHERS_LNAME"].' '.$regRutas["FNAME"]).'</option>';				
													}
													echo '</select>';
												} else {
													echo '<div></div>';
...
?>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="info-box bg-orange hover-expand-effect">
											...
											<div class="content">
												<div>
													<div class="text">% de Av</div>
													<div id="repdeAv">0 %</div>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="header">
										...
										<ul class="header-dropdown m-r--5">
											<li class="dropdown">
												...
												<ul class="dropdown-menu pull-right">
													<li id='exportpng11'><a href="javascript:void(0);">Descargar como PNG</a></li>
													<li id='exportpdf11'><a href="javascript:void(0);">Descargar como PDF</a></li>
												</ul>
											</li>
										</ul>
									</div>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="info-box bg-indigo hover-expand-effect">
											...
											<div class="content">
												<div>
													<div class="text">Contactos Reales</div>
													<div id="numeroContactosReales"></div>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="info-box bg-purple hover-expand-effect">
											...
											<div class="content">
												<div>
													<div class="text">Visita unica real</div>
													<div id="visitaUnicaReal"></div>
												</div>
											</div>
										</div>
									</div>
				...
				<!--inicio grafica 6 -->
				<!--start aqui  -->
				<div id="tblGraficas6" class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display:none;">
					<div class="col-xs-12 col-sm-12 col-md-5 col-lg-5">
						<div class="card">
							<div class="header">
								<div class="row">
										...
										<div class="display-flex">
											<?php
												if($tipoUsuario != 4){
													...
													<select id="sltRutasFarm" class="form-control add-m-b">
														<option value="">Todas las rutas</option>';
									...
									<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
										<div class="display-flex pull-right">
											<button type="button" class="btn bg-red waves-effect btn-red" id="btnExportarPlan2">
												<i class="material-icons">file_download</i>
												<span>Descargar</span>
											</button>
											<button type="button" class="btn bg-red waves-effect btn-red m-l-5" id="btnImprimirPlan2">
												<i class="material-icons">print</i>
												<span>Imprimir</span>
											</button>
										</div>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="info-box bg-indigo hover-expand-effect">
											...
											<div class="content">
												<div>
													<div class="text">Fichero Farmacia</div>
													<div id="ficheroMedico2"></div>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="info-box bg-purple hover-expand-effect">
											...
											<div class="content">
												<div>
													<div class="text">Visita Única a Farm</div>
													<div id="visitaUnicaFarm2"></div>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="info-box bg-teal hover-expand-effect">
											...
											<div class="content">
												<div>
													<div class="text">Visita Total a Farm</div>
													<div id="visitaTotal2"></div>
												</div>
											</div>
										</div>
									</div>
			...
			<div class="pull-center menu-charts">
				<div class="display-flex align-center">
					...
					<div id="btnDivGraficas6" class="bullet m-l-5">
						<i class="far fa-hospital font-22"></i>
						<p>Farmacias</p>
					</div>
