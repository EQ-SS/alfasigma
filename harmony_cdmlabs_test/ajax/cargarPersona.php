<?php
	...
	if(! $conn){
		...
		$queryPersona .= "plw.LATITUDE, plw.LONGITUDE, p.info as generales,p.info_shorttime as corto, 
		...
		//echo $queryPersona2;
		echo "<script>
				$('#lblDireccion2').text('".utf8_encode($reg2['direccion'])."');
				$('#lblCategoria2').text('".$reg2['categoria']."');
				$('#lblCedula2').text('".$reg2['cedula']."');
				$('#lblBrick2').text('".$reg2['brick_name']."');
				$('#lblRepresentante2').text('".utf8_encode($reg2['repre'])."');
				$('#lblConsultorio2').text('".utf8_encode($reg2['office'])."');
		...
		echo "<script>
				...
				$('#lblAPaterno').text('".utf8_encode($reg['paterno'])."');
				$('#lblCategoria').text('".$reg['categoria']."');
				$('#lblAmaterno').text('".utf8_encode($reg['materno'])."');
				$('#lblCedula').text('".$reg['cedula']."');
				$('#lblNombre').text('".utf8_encode($reg['nombre'])."');
				$('#lblFrecuencia').text('".$reg['frecuencia']."');
				$('#lblFrecPlan').text('".$reg['frecuencia']."');
				$('#lblFrecVisita').text('".$reg['frecuencia']."');
				$('#lblDireccion').text('".utf8_encode($reg['direccion'])."');
				...
				$('#lblRepresentante').text('".utf8_encode($reg['repre'])."');
		...
		while($plan = sqlsrv_fetch_array($rsPlanes)){
			...
			echo "$('#tblPlan tbody').append('<tr onClick=\"muestraPlan(\'".$plan['vispersplan_snr']."\');\"><td style=\"width:15%;\">".$plan['name']."</td><td style=\"width:10%;\">".$plan['user_nr']."</td><td style=\"width:15%;\">".$fecha_plan."</td><td style=\"width:15%;\">".$plan['Hora_Plan']."</td><td style=\"width:45%;\">".str_ireplace($buscar,$reemplazar,utf8_encode($plan['Objetivo']))."</td></tr>');";
		...
		while($visita = sqlsrv_fetch_array($rsVisitas)){
			...
			//echo "$('#tblVisitas tbody').append('<tr onClick=\"muestraVisita(\'".$visita['vispers_snr']."\',\'".$visita['idPlan']."\');\"><td style=\"width:15%;\">".$fecha_vis."</td><td style=\"width:10%;\">".$visita['time']."</td><td style=\"width:35%;\">".utf8_encode($visita['Rep'])."</td><td style=\"width:30%;\">".utf8_encode($visita['Tipo_Vis'])."</td><td style=\"width:10%;\">".substr($visita['ciclo'],0,5).substr($visita['ciclo'],8,2)."</td></tr>');";
		...
		/******************prescripciones*************/
		/*$queryPrescripciones = "select *, p.NAME as producto from PERSON_RX_PRODUCT pp
			inner join PRODUCT p on p.PROD_SNR = pp.PRODUCT_SNR
			where pers_snr = '".$id."'
			order by P.name";*/
		...
		echo '$("#lstProductoDatosPersonales").append(new Option("Seleccione", "0"));';
		$cuentaTablas = '';
		$contadorRx = 0;
		while($pres = sqlsrv_fetch_array($rsPrescripciones)){
			...
			//echo '$("#lstProductoDatosPersonales").append(new Option("'.$pres['PRODUCT_SNR'].'", "'.$pres['producto'].'"));';
			echo '$("#lstProductoDatosPersonales").append(new Option("'.str_replace('', ' ', $cadenaTest).'", "'.str_replace('', ' ', $cadenaTest).'"));';
			
			//echo "$('#tblPrescripciones').append('<tr><td><table width=\"100%\" border=\"0\" id=\"tbl".$pres['producto']."\"><tr><td class=\"negrita\" >".$pres['producto']."</td><td class=\"negrita\" >Quintil: </td><td class=\"negrita\" >Part: %</td></tr><tr><td colspan=\"3\"><div class=\"div-tbl-grey\"><table class=\"tblRxPersonas\"><thead class=\"bg-grey\"><tr class=\"align-center\"><td>Ranking</td><td>Producto</td><td>Market Share</td></tr></thead><tbody><tr><td>".$pres['POSITION']."</td><td>".$pres['producto']."</td><td>".$pres['MARKET_SHARE']."%</td></tr></tbody></table></td></tr></table></div></td></tr>');";
			echo "$('#tblPrescripciones').append('<div id=\"tbl".$cadenaTest."\"><div class=\"row col-indigo font-bold\"><div class=\"col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-0\"><span class=\"col-md-6 p-l-0 margin-0\">Período: ".$pres['periodo']."</span><span class=\"col-md-6 align-right margin-0\">Mdo: ".$pres['mdo']."</span></div><div class=\"col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-0\"><span class=\"col-md-4 margin-0 p-l-5\">Categoría: ".$pres['categoria']."</span><span class=\"col-md-4 align-center margin-0\">MS: ".$pres['ms']."</span><span class=\"col-md-4 align-right p-r-5 margin-0\">Prescribe: ".$pres['NUM_RX']."</span></div></div><div class=\"row\"><div class=\"col-lg-12 col-md-12 col-sm-12 col-xs-12\"><div class=\"div-tbl-grey\"><table class=\"tblRxPersonas\"><thead class=\"bg-grey\"><tr class=\"align-center\"><td style=\"width:30%;\">Ranking</td><td style=\"width:40%;\">Producto</td><td style=\"width:30%;\">Market Share</td></tr></thead><tbody><tr class=\"align-center\"><td style=\"width:30%;\">".$pres['POSITION']."</td><td style=\"width:40%;\">".$pres['producto']."</td><td style=\"width:30%;\">".$pres['MARKET_SHARE']."%</td></tr></tbody></table></div></div></div></div>');";
		}
		if($contadorRx == 0){
		...
		echo "</script>";
		//echo "frec: ".$reg['frecuencia']." ::: ".$reg['visitas']."<br>";
		//echo "<br>ina: ".$ina."<br>";
		//echo $queryPrescripciones;
		//echo "select PIMAGE_SNR from PERSIMAGE where PERS_SNR = '".$id."' <br>";
		//print_r($arrPasatiempos);
		//echo $queryHorario;
	}
?>