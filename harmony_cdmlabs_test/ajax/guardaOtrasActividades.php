<?php
	...
	function grabaVisita($conn, $idUsuario, $fecha, $comentarios, $arrActividades, $arrHoras, $fechaFin){
		$arrActividades = explode(",",$arrActividades);
		$arrHoras = explode(",",$arrHoras);
		...
		for($j=0;$j<count($arrActividades);$j++){
			...
			if(sqlsrv_num_rows($rsDRC) < 1){//no exite y la inserta
		...
		if($fechaFin != ''){
				...
				if('Sun' != date("D", $fechaDias)){
					...
					if($planVisita == 'plan'){
						if($repre != ''){
							...
							grabaPlan($conn, $idUsuario, $fechaNueva, $comentarios, $arrActividades, $arrHoras );
						}
					}else{
						if($repre != ''){
							for($usu=0;$usu<count($arrRepre);$usu++){
								grabaVisita($conn, $arrRepre[$usu], $fechaNueva, $comentarios, $arrActividades, $arrHoras,$fechaFin,$fechaIni);
							}
						}else{
							grabaVisita($conn, $idUsuario, $fechaNueva, $comentarios, $arrActividades, $arrHoras,$fechaFin,$fechaIni);
						}
					}
				}
			}
			...
			if($planVisita == 'plan'){
				if($repre != ''){
					...
					grabaPlan($conn, $idUsuario, $fecha, $comentarios, $arrActividades, $arrHoras);
				}
			}else{
				if($repre != ''){
					for($usu=0;$usu<count($arrRepre);$usu++){
						grabaVisita($conn, $arrRepre[$usu], $fecha, $comentarios, $arrActividades, $arrHoras,$fechaFin,$fechaIni);
					}
				}else{
					grabaVisita($conn, $idUsuario, $fecha, $comentarios, $arrActividades, $arrHoras,$fechaFin,$fechaIni);
				}
			}
		}
