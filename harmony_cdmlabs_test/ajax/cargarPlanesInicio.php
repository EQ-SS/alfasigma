	...
	if(! $conn){
		...
		while($plan = sqlsrv_fetch_array($rsPlanes)){
			...
			if($tipoUsuario == 4){
				...
				if($plan['tipo'] == 'p'){
					echo "$('#tblPlanesInicio tbody').append('<tr onclick=\"muestraPlan(\'".$plan['idPlan']."\')\"><td width=\"10%\" align=\"center\">".explode(" ",$plan["ruta"])[0]."</td><td width=\"50%\"><b>".$color."<br>".$plan["especialidad"]."</b><br>".$plan["street1"].", ".$plan["ZIP"].', '.$plan["colonia"]."<br>".$plan["brick"]."</td><td width=\"20%\" align=\"center\">".$fecha_plan."</font></td><td width=\"20%\" align=\"center\">".$plan["VRIJEME"]."</td></tr>');";
				}else{
