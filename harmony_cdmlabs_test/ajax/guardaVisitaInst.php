<?php
	...
	if(! $conn){
		...
		if(isset($_POST['productosStock']) && $_POST['productosStock'] != ''){
			...
			$productoStockPedido = $_POST['productoStockPedido'];
		...
		if($idVisita == ''){
			...
			$queryValida = "select * from visitinst where visit_date = '".$fecha ."' and inst_snr = '".$idInst."' and user_snr = '".$idUsuario."'";
			...
			//valida visita que no exista
			$queryValida = "select * from visitinst where visit_date = '".$fecha ."' and inst_snr = '".$idInst."' and visinst_snr <> '".$idVisita."' and user_snr = '".$idUsuario."'";
			//echo "<br><br><br><br><br><br>".$queryValida."<br><br>";
			$rsValida = sqlsrv_query($conn, $queryValida, array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
		...
		if($idVisita == ''){//nueva visita
			...
			if(sqlsrv_query($conn, $query)){
				//echo '<script>alert("Visita Guardada");';
			}else{
				echo $query."<br>";
		...
		if(! empty($_POST['productosSeleccionados'])){
			for($i=0;$i<count($arrProductosSeleccionados);$i++){
				...
				if(! sqlsrv_query($conn, $queryAgregaProductoSeleccionado)){
					...
					echo "<script>alertErrorTopProductos();</script>";
				}
			}
		}
		if(! empty($_POST['productosPromocionados']) && ! empty($_POST['cantidadProductosPromocionados'])){
		...
		if($idTipoInst == 2){//grabar stock
			...
			if($productosStock != ''){
				...
				$arrPedido = explode(",", substr($productoStockPedido, 0, -1));
				$arrSugerido = explode(",", substr($productoStockSugerido, 0, -1));
				for($i=0;$i<count($arrProductosStock);$i++){
					$queryStock = "insert into INST_STOCK_PRODFORM ( 
					INST_PRODFORM_SNR,
					INST_SNR,
					PRODFORM_SNR,
					QNTY,
			...
			if(isset($_POST['idProductoCompetidores']) && $_POST['idProductoCompetidores'] != ''){
				...
				for($i=0;$i<count($arrIdCompetidores);$i++){
					$queryCompetidor = "insert into INST_STOCK_PRODFORM ( 
					INST_PRODFORM_SNR,
					INST_SNR,
					PRODFORM_SNR,
					QNTY,
		...
		//echo "<br><br>select * from binarydata where table_nr = '34' and rec_key = '".$idVisita."'";
	}
?>