<?php
	...
	if(! $conn){
		...
		$rsExiste = sqlsrv_query($conn, "select * from vispersplan where plan_date = '$fechaPlan' and pers_snr = '$idPersona'", array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
		...
		if(sqlsrv_query($conn, $query)){
			...
			//echo $queryPlanes."<br>";
				...
				while($plan = sqlsrv_fetch_array($rsPlanes)){
					...
					echo "$('#tblPlan tbody').append('<tr onClick=\"muestraPlan(\'".$plan['vispersplan_snr']."\');\"><td style=\"width:15%;\">".$plan['NAME']."</td><td style=\"width:10%;\">".$plan['user_nr']."</td><td style=\"width:15%;\">".$fecha_plan."</td><td style=\"width:15%;\">".$plan['Hora_Plan']."</td><td style=\"width:45%;\">".$plan['Objetivo']."</td></tr>');";
				}
