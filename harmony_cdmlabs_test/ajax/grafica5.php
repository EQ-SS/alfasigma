<?php
	...
	if($tipoUsuario == 2){
		$queryGrafica5 .= "select a.sort_num, a.ESP, sum(a.regist) as regist, sum(a.visitados) as VISITADOS from ( ";
	}
	$queryGrafica5 .= "select isnull(spec_target.sort_num,999) as sort_num, isnull(spec.name_short,'OTRAS') as ESP, 
		(select count(*) 
		from person p 
		...
		where p.rec_stat=0 and status.name='ACTIVO' and p.spec_snr=spec.clist_snr and psw.rec_stat=0 and plw.rec_stat=0 and ut.rec_stat=0 and i.rec_stat=0 
		...
		inner join COMPLINE cp on cp.CLINE_SNR = spec_target.CLINE_SNR
		inner join USERS u on u.CLINE_SNR = cp.CLINE_SNR 
		where spec.clib_snr in (select clib_snr from codelistlib where table_nr=19 and list_nr=1) 
		and spec.rec_stat=0 and spec.status=1 and u.rec_stat=0
		and u.user_snr in ('".$ids."') ";
	
	if($tipoUsuario == 4){
		$queryGrafica5 .= " order by sort_num ";
	}else{
		$queryGrafica5 .= " ) a
			group by a.sort_num, a.ESP 
			order by sort_num ";
	}
