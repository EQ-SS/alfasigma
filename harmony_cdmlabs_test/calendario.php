					...
					<div class="header">
						<div class="row">
							...
							if($tipoUsuario != 4){
								...
								$repre = sqlsrv_query($conn, "select user_snr, user_nr as ruta, lname + ' ' + fname as nombre from users where user_snr in ('".$ids."')");
								while($rep = sqlsrv_fetch_array($repre)){
									$nameDividido = explode('-',$rep['nombre']);
									$nameSinNR = count($nameDividido) >1 ? $nameDividido[1] : $nameDividido[0];
									$rep['nombre'] = $rep['ruta'].' - '.$nameSinNR;
