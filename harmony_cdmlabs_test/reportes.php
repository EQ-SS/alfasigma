								...
								<div class="card" style="box-shadow: none; border: 1px #f1f1f1 solid;">
									...
									<div class="body table-responsive" style="height: 550px;">
										<table class="table table-hover table-striped pointer" id="tablaReportes">
											...
											<tr class="esteReporte">
												<td id="reporteInv"
													onclick="criteriosReporte(this.id,'repre','reporteInventario.php');">
											...
											<tr class="esteReporte">
												<td id="reporteFarmVisDia"
													onclick="criteriosReporte(this.id, 'fecIni,fecFin,repre','reporteFarmaciasVisitadasDia.php');">
													Reporte de farmacias visitadas por día
												</td>
											</tr>
											<tr class="esteReporte">
												<td id="reporteLlenaCampos"
													onclick="criteriosReporte(this.id, 'repre','reporteLlenadoCampos.php');">
													Reporte de llenado de campos
												</td>
											</tr>
										</table>
									</div>
									...
									<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
										<div id="repre" style="display:none;">
											...
											if($tipoUsuario != 4){
												...
												$queryRepres = "select user_snr, user_nr as ruta, lname + ' ' + MOTHERS_LNAME + ' ' + fname as nombre from users where USER_SNR in ('".$ids."') ";
												$repre = sqlsrv_fetch_array(sqlsrv_query($conn, $queryRepres));
												$nameDividido = explode('-',$repre['nombre']);
												$nameSinNR = count($nameDividido) >1 ? $nameDividido[1] : $nameDividido[0];
												$repre['nombre'] = $repre['ruta'].' - '.$nameSinNR;
