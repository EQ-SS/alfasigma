<?php
	...
	$tam = array(100,350,350,100,200,450,550,450,100,250, 250,200,100,150);
	...
	$query = "Select
		...
		and I.creation_timestamp between '".$fechaI."' and '".$fechaF."' 
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoHospitalesAltas".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado Altas de Hospitales");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoHospitalesAltas.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO ALTA DE HOSPITALES'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO ALTAS DE HOSPITALES</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO ALTAS DE HOSPITALES')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoHospitalesAltas');
