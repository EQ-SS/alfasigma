<?php
	...
	$tam = array(350,350,350,200,200,250,450,450,100,200, 200,150,150,100,250,250,150,150,250,100, 200,200,200,150,250,100);
	...
	$qMedicos = "Select
		...
		P.tel2 as Tel2,
		P.mobile as Celular,
		cast(cast(aps.date_change as DATE) as nvarchar(10)) as Fecha_Baja,
		EB.name as 'Estatus de baja',
		case when P.REC_STAT = 0 then 'REACTIVADO' else '' end as Reactivado
		 
		from person P
		inner join perslocwork PLW on P.pers_snr = PLW.pers_snr
		inner join pers_srep_work PSW on PSW.pers_snr=PLW.pers_snr and PLW.pwork_SNR=PSW.PWORK_SNR
		...
		left outer join person_approval pa on pa.p_pers_snr = p.pers_snr 
		inner join approval_status aps on aps.pers_approval_snr = pa.pers_approval_snr 
		left outer join codelist EB on pa.plw_del_status_snr = EB.clist_snr
		...
		and aps.date_change between '".$fechaI."' and '".$fechaF."'
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoMedicosBajas".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado Bajas de Medicos");
	}
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO BAJAS DE MÉDICOS')
