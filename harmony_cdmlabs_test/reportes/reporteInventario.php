<?php
	...
	$tam = array(350,200,450,200,200,100,100,100);
	...
	$qMedicos = "select
		...
		/*,ROW_NUMBER() over (PARTITION BY P.name order by P.name) as Num_Reg*/
		P.name as Producto,
		PF.name as Presentación,
		PBATCH.name as Lote,
		...
		/*left outer join CYCLES C on C.CYCLE_SNR = SPU.CYCLE_SNR and C.rec_stat=0 */
		...
		/*and U.user_nr in ('1001')*/
		/*and P.PROD_SNR in ({?product})*/
	...
	while($regMedico = sqlsrv_fetch_array($rsMedicos)){
		...
		for($j=0;$j<sqlsrv_num_fields($rsMedicos);$j++){
			...
			if($j == 7){
				$regMedico[$j] = $regMedico[5] - $regMedico[6];
			}
