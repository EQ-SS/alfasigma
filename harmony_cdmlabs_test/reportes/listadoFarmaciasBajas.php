<?php
	...
	$tam = array(100,350,350,100,200,250,450,550,450,100, 250,250,150,100,150);
	...
	$query = "Select
		...
		cat.name as Categ,
		CAST(CAST(aps.approved_date as DATE) as VARCHAR(10)) as Fecha_Baja
		 
		from Inst I
		inner join Approval_status as aps on aps.record_key=I.inst_snr 
		...
		and U.status=1
		and U.user_type=4
		/*and I.status_snr = 'F01844BD-3288-4F2A-9C56-382C3B78D6F4' */
		and U.user_snr in ('".$ids."')
		and aps.approved_date between '".$fechaI."' and '".$fechaF."' 
		and aps.rec_stat=0 
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoFarmaciasBajas".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado Bajas de Farmacias");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoFarmaciasBajas.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO BAJAS DE FARMACIAS'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO BAJAS DE FARMACIAS</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO BAJAS DE FARMACIAS')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoFarmaciasBajas');
