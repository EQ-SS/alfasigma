<?php
	...
	$tam = array(350,350,350,100,100,100,350,450,100,200, 200,200,200,100,250,250,150,150,150,150, 150,150,200,200,150);
	...
	$qMedicos = "Select
		...
		P.tel2 as Tel2,
		P.mobile as Celular,
		cast(cast(P.creation_timestamp as DATE) as nvarchar(10)) as Fecha_Alta
 
		from person P
		inner join perslocwork PLW on P.pers_snr = PLW.pers_snr
		inner join pers_srep_work PSW on PSW.pers_snr=PLW.pers_snr and PLW.pwork_SNR=PSW.PWORK_SNR and PSW.rec_stat=0
		...
		$qMedicos .= " order by Cl.name,U.lname,U.fname,P.lname,P.mothers_lname	,P.fname ";
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoMedicosAltas".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado Alta de Medicos");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoMedicosAltas.xls");
	...
	if($tipo == 3){
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO ALTA DE MÉDICOS'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO ALTA DE MÉDICOS</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO ALTAS DE MÉDICOS')
