<?php
	...
	$tam = array(100,350,350,100,200,250,450,550,450,100, 300,250,200,100,250,250,150,150,150,150 ,150,250,150,100,150,250,250,550);
	...
	while($registro = sqlsrv_fetch_array($rsInst)){
		...
		for($j=0;$j<sqlsrv_num_fields($rsInst);$j++){
			...
			if($j == 20){
				if( ($registro[16] != '') && ($registro[16] != '0.0') ){
					if( ($registro[18] != '') && ($registro[18] != '0.0') ){
						$total_latitud = abs( number_format( $registro[16] - $registro[18] , 4) );
					}else{
						$total_latitud = 0;
					}
				}else{ 
				...
				if( ($registro[17] != '') && ($registro[17] != '0.0') ){
					if( ($registro[19] != '') && ($registro[19] != '0.0') ){
						$total_langitud = abs( number_format( $registro[17] - $registro[19] , 4) );
					}else{
						$total_langitud = 0;
					}
				}else{ 
				...
				$registro[$j] = $total_latitud + $total_langitud;
			}
			if($j == 21){
				if( ($registro[16] != '') && ($registro[16] != '0.0') && ($registro[17] != '') && ($registro[17] != '0.0') ){
					if( ($registro[18] != '') && ($registro[18] != '0.0') && ($registro[19] != '') && ($registro[19] != '0.0') ){
						if($registro[20] < 0.001){ 
							$registro[$j] = 'SIMILAR'; 
						}
						if( ($registro[20] >= 0.001) && ($registro[20] < 0.01) ){ 
							$registro[$j] = 'DIFERENTE'; 
						}
						if( ($registro[20] >= 0.01) && ($registro[20] < 0.1) ){ 
							$registro[$j] = 'MUY DIFERENTE'; 
						}
						if($registro[20] >= 0.1){ 
							$registro[$j] = 'EXTREMADAMENTE DIFERENTE'; 
						}						
					}else{
						$registro[$j] = 'SIN GEOLOCALIZAR VISITA';
					}
				}else{ 
					$registro[$j] = 'SIN GEOLOCALIZAR FARMACIA';
				}				
			}
