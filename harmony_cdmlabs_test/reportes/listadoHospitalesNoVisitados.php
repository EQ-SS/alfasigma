<?php
	...
	$tam = array(100,350,350,100,200,450,550,450,100,300, 250,200,100,250,250,150,150);
	...
	if($tipo == 2){//excel sin formato
		$nombreArchivo = "../archivos/ListadoHospitalesNoVisitados".date("dmYHis").".xlsx";
		$spread = new Spreadsheet();
	
		$spread->getProperties()
					->setCreator("Smart-Scale")
					->setTitle("Listado")
					->setDescription("Listado de Hospitales No Visitados");
	}
	if($tipo == 1){//excel con formato
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=listadoHospitalesNoVisitados.xls");
		...
		$pdf->Cell(40,5,utf8_decode('LISTADO DE HOSPITALES NO VISITADOS'));
		$pdf->Ln();
		$pdf->SetFont('Arial','B',10);
		...
		$tabla = '<table border="0">
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="10" class="nombreReporte">LISTADO DE HOSPITALES NO VISITADOS</td>
						</tr>
						<tr>
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
            ->setCellValue('A1', 'LISTADO DE HOSPITALES NO VISITADOS')
	...
	if($tipo == 2){
		$spread->setActiveSheetIndex(0)
			...
			->setTitle('ListadoHospitalesNoVisitados');
